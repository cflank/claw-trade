...
已删除数据网关 本体已删除，不是目标运行时。本文继承的只有三类思想：
...
- 已删除数据网关 全量应用、CLI、Workspace、Excel；
- 已删除数据网关 运行时作为数据层核心；
- 已删除数据网关 的对象模型作为仓库结构；
- 旧 已删除数据网关 前缀作为目标表名、模块名或证据链名。
...
- 按 market/data_type/granularity/source_role 过滤；
- 缺凭证的 provider 不出网，记录 `credential_missing` skip/attempt；
- 读取 `ProviderPlugin.capabilities()` 的能力快照；
- 按插件能力中的 `priority_rank` / 默认优先级排序；
- 保持官方源不可被普通数据源替代。
- 不因为 `coverage_fields` 未确认或字段声明不完整就提前丢 provider。字段覆盖只能用于排序、合并、归一化后验收和缺口解释，不能作为选择阶段的硬过滤。
...
def select_candidates(self, gaps: Sequence[DataGap], plan: QueryPlan) -> tuple[ProviderCandidate, ...]:
    ...
    for gap in gaps:
        ...
        matched = [
            ...
            and credential_available_or_record_skip(cap)
        ]
...
- 按 source/provider 命名空间、credential scope、window 持久化共享配额计数；429/quota 后的冷却按 endpoint key 记录；
- 支持 `safety_margin`；
...
- 需要引入 已删除数据网关 本体运行时作为主依赖。
- 需要用旧 已删除数据网关 前缀作为目标表名、模块名或证据链名。
