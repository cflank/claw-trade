...
- OpenBB/data_gateway 仍是唯一外部取数入口、唯一 provider 接口层、唯一数据 MCP、唯一 provider evidence 记录者。
- CRYPTO worker 仍只通过 `claw_get_market_pack` 获取自然语言 market pack；worker 不直接读取旧 BB MCP、CryptoLens raw JSON、OpenBB atomic/admin/discovery tools、Mongo raw/cache/debug envelope 或 OpenViking protocol。
- CryptoLens analysis 只消费 OpenBB normalized crypto bundle，不出网、不读 provider key、不调用 OpenBB、不访问 Mongo、不写 BUY/HOLD/SELL、仓位、执行建议、PM rating 或最终投资裁决。
...
| T-CL-3 | 建立旧路径、旧工具、旧 executor、OpenBB atomic/admin/discovery import-block 基线 | T-CL-1 | 可与 T-CL-2 并行 | 禁止项扫描、import-block 测试基线 |
| T-CL-4 | 定义 OpenBB normalized crypto bundle 合同 | T-CL-1 | 可与 T-CL-5 前半并行 | bundle/input/status/dat gap/conflict/ref 合同 |
| T-CL-5 | 扩展 OpenBB CRYPTO provider plan 和 adapters | T-CL-4 | 子域可并行 | market/OHLCV/derivatives/liquidation/onchain/macro/events/AHR999 provider evidence |
| T-CL-6 | 按旧 BB 纯分析口径 Python 重写 CryptoLens | T-CL-2、T-CL-4 | 可与 T-CL-5 后半并行 | `src/claw_trade/data_gateway/analysis/crypto_lens/**` |
| T-CL-7 | 实现 CryptoLens adapter、input/output/evidence | T-CL-4、T-CL-6 | 串行于 T-CL-6 后 | `analyze_openbb_crypto_lens_bundle`、`crypto_lens_analysis_evidence` |
...
- T-CL-5 的 provider 子域可以拆分并行，但所有子域必须统一走同一个 OpenBB/data_gateway provider contract。
...
- 读取 `docs/CryptoLens接入方案.md`、`docs/数据源openbb引入方案.md`、当前 CRYPTO final report 证据目录。
- 不修改运行代码。
...
- 证据中明确当前 CRYPTO market source 是否仍只有 `openbb_yfinance/crypto_price_historical` 或等价窄覆盖。
- 明确后续所有任务不得恢复旧 BB 直连。
...
目标：先建立负面证明，防止迁移过程中旧 BB、旧工具、旧 executor 或 OpenBB atomic/admin/discovery 工具 silent fallback。
...
- tool schema 测试覆盖 OpenBB flag 下 frontline 只暴露 canonical pack tools，下游 worker 不暴露数据工具。
- report worker 的 model-visible tool schema 只允许 `claw_get_*_pack`；OpenBB provider/admin/discovery/debug 能力可保留在控制层/CLI/UI 调试入口，但不得进入 report worker model-visible schema。
...
- 额外扫描证明 OpenBB schema 模式下没有旧 BB MCP 参与 CRYPTO pack 成功路径。

### T-CL-4：OpenBB normalized crypto bundle 合同

目标：定义 OpenBB 喂给 CryptoLens 的唯一输入合同。
...
### T-CL-5：OpenBB CRYPTO provider plan 和 adapters

目标：把 CRYPTO market 从 yfinance-only 扩展为多域 OpenBB/data_gateway provider plan。
...
- OpenBB provider 失败时写真实 gap，CryptoLens 不补数。
...
目标：实现 OpenBB normalized bundle 到 CryptoLens analysis result 的唯一入口，并写独立 analysis evidence。
...
- 输出可 hash。
- 缺口原样传递或进一步细化。
- input 映射必须来自真实 normalized bundle + attempt/normalized refs，不接受 ticker/market/date 的模型侧覆盖来篡改 runtime 实际值。
- 不调用网络、不读取 provider key、不访问 Mongo、不调用 OpenBB。
- 不输出 BUY/HOLD/SELL、仓位、执行建议、PM rating 或最终投资裁决。

证据：

- `crypto_lens_analysis_evidence` 记录 run_id、call_id、worker_id、analysis_id、engine name/version/source hash、input hash、normalized refs、output hash、no-network assertion、data gaps/conflicts、analysis result ref。
- 不得把 `crypto_lens_analysis_evidence` 命名或伪装成 OpenBB HTTP/raw provider evidence。

验收：

- no-network、no-key、no-Mongo、no-OpenBB tests。
- analysis evidence 与 OpenBB provider HTTP/raw evidence 命名隔离 tests。
- CryptoLensAnalysisResult 全字段合同测试通过（包含 success/partial/failure 分支字段完整性）。
- 真实 attempts/normalized refs 进入 CryptoLens input 的集成测试通过。
...
- downstream workers 不看到 OpenBB 数据工具。
- provider payload 不含旧 BB MCP 原子工具、OpenBB atomic/admin/discovery/raw/debug/cache tools。
...
目标：让 final report claim 可追溯到 CryptoLens analysis evidence 和 OpenBB provider HTTP/raw evidence。
...
- OpenBB 某域失败：写 attempt、HTTP/raw evidence if request sent、data gap，bundle 该域 missing/partial/error，CryptoLens 只基于剩余域分析。
- CryptoLens analysis 失败：OpenBB evidence 保留，`crypto_lens_analysis_evidence` 写失败状态，market pack readiness 降级，worker 不伪造指标分析。
- OpenBB 数据不足：指标 partial/insufficient，不把未计算写成中性。
...
- OpenBB adapter 读取本地 key 成功但 evidence 不泄漏 key 值。
...
- 开工冻结门证据齐备：OpenBB submodule URL/commit/tag/license 摘要、审批记录、extension/shim 形态审批记录已落在设计证据目录。
...
- openbb_runtime_marker、openbb_extension_version。
...
| OpenBB 唯一外部数据入口和 provider evidence 边界 | 1、4、5、6.1、7 | T-CL-4、T-CL-5、T-CL-10、T-CL-12 |
| 禁止旧 BB MCP、旧 BB 直连、CryptoLens 再调 OpenBB | 1、4、9.4、15 | T-CL-2、T-CL-3、T-CL-6、T-CL-11 |
| `/report` 到 final report 端到端时序 | 5.1 | T-CL-8、T-CL-9、T-CL-10、T-CL-12 |
| OpenBB CRYPTO 多域 provider adapters | 6.1、8 | T-CL-5、T-CL-11、T-CL-12 |
| NormalizedCryptoMarketBundle 作为唯一输入 | 6.2、6.3 | T-CL-4、T-CL-7 |
...
| OpenBB provider 失败、CryptoLens 失败、数据不足、cache 命中 | 10 | T-CL-8、T-CL-11 |
...
| 旧 BB MCP server | `bb_crypto_data`、`bb_crypto_data__build_trade_context`、`BB_MCP_SERVER_PATH`、`BB_MCP_CWD`、`BB_MCP_HTTP_*` | 旧 BB 会自己出网取数，破坏 OpenBB 唯一入口 | T-CL-3、T-CL-11、T-CL-12 |
| 旧 BB live provider fetch | `mcp/crypto-data-mcp/src/domains/live.ts`、CoinGlass/Binance/Bybit/FRED/CoinGecko direct fetch | provider 取数必须进入 OpenBB evidence chain | T-CL-2、T-CL-5、T-CL-6、T-CL-11 |
| 旧资料包 fallback | `crypto_market_data_pack.py`、`market_market_data_pack`、`crypto_market_data_pack`、`frontline_data_pack` | 不能在 OpenBB 失败后 silent fallback | T-CL-3、T-CL-8、T-CL-11 |
| 旧 provider executor | `frontline_data_pack.provider_executor`、旧 `provider_executor` import slice、旧 `claw_trade.providers` | 目标态使用 OpenBB/data_gateway provider contract，不复用旧 executor | T-CL-3、T-CL-5、T-CL-11 |
...
| OpenBB atomic/admin/discovery/debug tools（model-visible） | `provider.`、`admin.`、`discovery.`、`activate_tools`、`execute_prompt`、`list_providers`、`cache`、`raw`、`debug` | report worker 模型不能直接调用这些工具；控制层/CLI/UI 调试入口可存在但不进 worker schema | T-CL-3、T-CL-9、T-CL-12 |
...
| CryptoLens 再调 OpenBB/provider | CryptoLens module import/call OpenBB runtime、pack endpoint、provider adapters | 会形成 OpenBB -> CryptoLens -> OpenBB 绕圈 | T-CL-7、T-CL-11 |
...
1. `claw_get_market_pack` 在 CRYPTO 下真实走 OpenBB provider plan 取数，RunProviderPlan 不 prefetch、不写 remote success。
2. 所有外部 provider 请求都有 OpenBB HTTP/raw evidence，并有 attempt、normalized ref、cache receipt、rate-limit 或错误记录。
3. CryptoLens analysis engine 来自 claw-trade 仓库内部代码，在 report runtime 中不出网、不读 provider key、不访问 Mongo、不调用 OpenBB。
4. CryptoLens analysis input 只来自 OpenBB normalized crypto bundle。
5. CryptoLens analysis evidence 独立命名为 `crypto_lens_analysis_evidence`，不能替代 OpenBB HTTP/raw provider evidence。
6. worker 只看到 `claw_get_market_pack` 和自然语言 market pack，不看到旧 BB MCP、OpenBB atomic/admin/discovery/raw/debug/cache tools、raw JSON、Mongo raw/cache/debug envelope 或 OpenViking protocol。
7. BTC fresh/live report 恢复 CryptoLens 指标分析密度；缺失域真实写 data gaps，不能把未覆盖写成成功覆盖。
8. final report evidence chain 能追到 PM L1、market L1、CRYPTO market pack audit、CryptoLens analysis evidence、OpenBB normalized refs、provider attempts、HTTP evidence、raw evidence。
9. `/mnt/d/src/BB` 不存在或不可访问、`BB_MCP_SERVER_PATH`/`BB_MCP_CWD` 为空或无效时，目标态 CRYPTO market pack 仍能运行且不启动外部 BB MCP。
10. OpenBB 真实缺口只通过 readiness/data gaps 表达；不得把“旧 BB 路径不可用”包装成完成口径。
...
- 需要让 CryptoLens 自己出网、读取 provider key、调用 OpenBB、访问 Mongo，才能补齐指标。
...
- OpenBB/data_gateway 无法承载某个必需 provider 域，且没有批准的缺口表达策略。
- 需要把 OpenBB atomic/admin/discovery/raw/debug/cache tools 暴露给 worker。
...
- 需要把 CryptoLens 代码放进 `third_party/openbb` 写 claw-trade 业务逻辑。
