...
| data_gateway 是统一数据层入口 | 3、6、13 | `src/claw_trade/data_gateway/**` | `DataAPI.get_data/get_data_batch`、`ProviderRegistry` | `tests/contracts/test_data_gateway_cutover.py` | evidence 证明 `DataRequest -> DataResult`，无旧 provider import |
...
| 新增三个 pack tool | 6 | pack/tool wrapper | `claw_get_policy_pack` 等 | `test_frontline_tool_protocol.py` / `test_tool_registry_contract.py` | model-visible tools 只暴露 canonical pack 工具 |
...
| 用户 provider 失败后尝试同组系统源 | 9 | `data_gateway/coordination/**`、`providers/**` | provider selection / batch plan | `test_provider_execution.py` | attempts 显示用户失败和系统源尝试 |
| 所有尝试写 provider attempt | 9、13 | `data_gateway/ingest/attempt_log.py` | `AttemptLog` | `test_mongo_attempts.py` | Mongo `provider_attempts` 有每个候选记录 |
...
| OpenClaw payload 与数据层 provider evidence 不混淆 | 13、14 | evidence collectors | named evidence refs | provider evidence audit tests | LLM provider payload 与 data_gateway raw/attempt refs 分离 |
...
| `data_gateway` | 统一数据层入口、provider plugin/registry、provider attempt、raw/normalized/cache/readiness/data gaps、pack 生成 | worker prompt、DAG、PM 决策、OpenViking material authority |
...
/report
  ...
  -> ProviderRegistry / ProviderPlugin
  -> Mongo provider raw/normalized/cache/attempt evidence
...
### 7.1 `a-stock-data` 到 data_gateway provider plugin 的映射原则

`a-stock-data` 不进入 worker tool，不作为独立 MCP，不作为旧 provider executor。每个列入设计的可用 A股源都应转成 `data_gateway` provider plugin，能力由 `ProviderPlugin.capabilities()` 声明：
...
Phase 1 不做“先做一部分再补范围”。文档 7.2/7.2A 已列出的七域 coverage_group 都进入工程任务范围。实现时按上表作用域逐组接入；真实运行中先尝试用户已验证并启用的同组 provider，再尝试系统默认 provider。所有尝试都必须写 provider attempt 和 gaps/readiness。
...
- `a-stock-data` 已经给出接口地址和示例代码；缺口不是“找不到接口”，而是 claw-trade 还没有把这些接口做成 `data_gateway` provider plugin/adapter。
- 每个要接入的接口都必须在本仓经 `data_gateway` provider plugin/adapter 实测一次，保存样本：请求参数、返回字段、单位、空值行为、失败状态。
...
| 缺口 | 说人话 | 为什么重要 | 关闭证据 |
|---|---|---|---|
| 参考接口样本/探针样本 | `a-stock-data` 代码里有地址和参数，但本仓还没按 data_gateway provider plugin 跑一遍 | 避免照抄后字段漂移、单位错误或接口空返回 | 每个计划源 1 个可取证样本：请求、状态、字段、单位、失败/空返回形态；adapter 实测放入 Phase 1 验收 |
...
- 用户 enabled candidate 优先尝试，但失败后同组系统源必须继续尝试。
...
- 验收以当前 run plan / provider attempts + run trace 对齐为准，不以 UI 当前页状态替代。
...
- `provider_http_evidence`：只指 data_gateway/provider adapter 调外部 provider 的 HTTP 证据。
- `provider_raw_evidence`：只指外部数据 raw payload/hash/ref。
...
- 图表所用字段必须能追溯到 `normalized_datasets` 和 source refs，且保留单位、日期和来源。
...
2. 同 call 的 data_gateway `provider_attempts` / `raw_payloads` refs；
3. 同 worker 的 L1 输出。
...
7. data_gateway 根据数据请求和 provider 能力执行 provider candidates。
8. 每个 provider attempt 写 Mongo：raw ref、normalized ref、cache receipt、attempt。
...
| 步骤 | 证据 | 存储建议 |
|---|---|---|
| data request / batch plan | `DataRequest` / `ProviderBatchPlan` | run evidence 或 `dataset_manifests` |
| raw payload | raw hash/ref | Mongo `raw_payloads` 或对象存储 |
| normalized | `NormalizedResult`/bundle | Mongo `normalized_datasets` |
| attempt | `ProviderAttempt` | Mongo `provider_attempts` |
| cache | `CacheReceipt` | Mongo `provider_result_cache` |
...
- data_gateway raw/attempt evidence 证明“外部数据源调用和原始数据是什么”。
...
| 样本类型 | 最低内容 | 用途 | 验收证据 |
|---|---|---|---|
| provider response sample | 每个 Phase 1 计划源至少 1 个成功响应；包含请求参数、状态码、关键字段、单位、时间戳和来源 URL/hash | normalizer、reader_brief、chart refs | fixture 文件 + provider raw/attempt ref 对照 |
...
| priority/conflict sample | 同组系统源、用户源、官方源混合排序样本；至少一个用户源失败后系统源成功 | priority 作用域和替换链 | data request/batch plan + attempts 顺序 |
...
| `test_official_original_attempt_semantics` | 同组 `official_original + USER_PREFERRED`，官方失败样本 | 官方原文 attempt 语义 | `official_original` 必有 attempt；失败写 gap；搜索源不能冒充官方事实 | `provider_attempts` 样本、pytest 输出 | `uv run pytest tests/unit/data_gateway/test_provider_registry.py -k official_original_attempt` |
...
| `test_fundamental_peg_fields_cn_a` | 一致预期 EPS、机构覆盖数、预测增速、当前价样本；另含缺 EPS/缺增速样本 | PEG/估值消化字段 | forward PE/PEG/PE 消化有 refs；缺核心输入时写 gap，不补算 | `normalized_datasets` 样本 + pytest 输出 | `uv run pytest tests/unit/data_gateway/test_fundamental_pack_cn_a.py -k peg` |
...
| 测试 | 输入 | 验证点 | 期望输出 | 证据路径/collection | 命令 |
|---|---|---|---|---|---|
| `test_policy_pack_cn_a` | CN_A data requests + policy provider fixtures | pack 结构与读者正文 | `reader_brief_md` 为自然语言，含 readiness/gaps/refs | `provider_attempts`、`normalized_datasets` | `uv run pytest tests/integration/data_gateway/test_policy_pack_cn_a.py` |
| `test_hot_money_pack_cn_a` | 龙虎榜/资金流多候选状态 | coverage_group 执行与替换链 | 用户源失败后继续同组系统源，attempt 完整 | `provider_attempts`、raw refs | `uv run pytest tests/integration/data_gateway/test_hot_money_pack_cn_a.py` |
| `test_lockup_pack_cn_a` | 解禁/股东户数/大宗样本 | normalization + gaps | bundle 字段完整；缺口可解释 | `normalized_datasets`、`provider_attempts` | `uv run pytest tests/integration/data_gateway/test_lockup_pack_cn_a.py` |
| `test_pack_tools_cn_a_extensions` | pack tool wrapper | route 与 tool schema | 仅 canonical pack tool 暴露给对应 frontline worker（含三新域） | wrapper audit + pytest 输出 | `uv run pytest tests/contracts/test_frontline_tool_protocol.py tests/contracts/test_tool_registry_contract.py` |
| `test_run_provider_plan_cn_a_domains` | CN_A `/report` request | run plan/data request 冻结 | 7 域、`remote_prefetch_allowed=false`、写入 `provider_config_version` | run evidence / `dataset_manifests` | `uv run pytest tests/integration/data_gateway/test_run_provider_plan_snapshot.py -k cn_a_domains` |
| `test_active_run_provider_config_version_not_hot_reloaded` | run 中 UI 修改 provider catalog | active run 版本语义 | active run 使用旧 version；新 version 仅下一次 run 生效 | run evidence + run trace | `uv run pytest tests/integration/data_gateway/test_run_provider_plan_snapshot.py -k not_hot_reloaded` |

### 14.3 Provider 准入与安全测试矩阵

| 测试 | 输入 | 验证点 | 期望输出 | 证据路径/collection | 命令 |
|---|---|---|---|---|---|
| `test_declarative_provider_policy_enabled_candidate` | policy manifest | admission 状态机 | `validated -> enabled_candidate` 后可进 registry | validation receipt evidence | `uv run pytest tests/unit/data_gateway/test_provider_admission.py -k enabled_candidate` |
...
| provider evidence audit | 同一 live run | 证据不混淆 | `openclaw_llm_provider_payload` 与 data_gateway raw/attempt refs 分离，run/dispatch/call 可对齐 | `provider_attempts`、`raw_payloads`、run calls evidence | `uv run pytest tests/contracts/test_provider_evidence_audit_boundary.py` |
| 图表与 root cause 验收 | 同一 live run | charts when available | 有图表则有 chart ref；缺图必须有 `chart_readiness/root_cause`，不得假图或静默成功 | `runs/<run_id>/reports/assets`、pack audit、`normalized_datasets` | `uv run pytest tests/integration/data_gateway/test_market_pack_cn_a.py -k chart_readiness` |
...
| 三新增 pack provider evidence | 同一 run | data_gateway provider 调用 | attempts/raw/normalized/cache evidence 都可追溯 | Mongo `provider_attempts/raw_payloads/normalized_datasets/provider_result_cache` | `uv run pytest tests/contracts/test_provider_evidence_audit_boundary.py -k cn_a_new_packs` |
...
核对门目标：在 Phase 1 代码变更前确认已批准的架构边界、当前 data_gateway 合同、SSRF 策略、baseline 位置和 provider 作用域。provider 真实字段、adapter 实测、pack endpoint 测试、readiness negative test、chart readiness contract 落地和 live/fresh provider payload 证明都属于 Phase 1 验收，不得倒灌为 Phase 0 阻断门。
...
2. 任一 provider 失败时记录 attempt/gap/readiness，而不是绕过 data_gateway 或静默 fallback。
...
- data_gateway pack/tool/provider response tests
...
- data_gateway 合同漂移、SSRF 策略被放松，或新增 pack 接口无法通过当前 data_gateway/tool schema 承载。
- 任一新增 pack 必须绕过 data_gateway 才能取数。
...
- 文档列出的 A股源参考样本已转成 data_gateway provider plugin/adapter 实测证据，覆盖成功、失败/空返回、关键字段、单位和来源引用。
- 7 个 canonical pack endpoint/tool、`chart_readiness`、official_original negative readiness 和 data_gateway pack/tool tests 通过。
...
|---|---|---|---|
| 商业/SaaS 使用 A股源 | 当前范围按项目内研究工作流推进；商业分发、多用户展示和付费源托管另审 | 需要商业发布、SaaS、多用户数据展示或托管用户密钥 | 商业发布和商业 UI |
...
- data_gateway pack/tool tests、provider adapter 成功/失败/空返回样本测试、provider response fixture 对照。

### 16.2 未来发布/未来扩展才需要关闭

| 问题 | 为什么不是 Phase 1 阻断项 | 需要拍板的时点 |
|---|---|---|
| 后续商业/SaaS 使用 A股源 | 当前范围只批准个人研究；商业分发、SaaS、多用户数据展示需要重新审 license 和源条款 | 商业发布前 |
