# TradingAgents-CN final provider prompt/messages (最终报告轮)

[user]
请输出你的前线分析报告。

[assistant]
[
  {
    "signature": "d45a249944b6b9385ca35309f9e9df93a9adc4cc36a2326b14496a01f2ea0efd",
    "thinking": "用户要求我输出分析报告。根据工作流程，我需要：\n1. 首先调用 get_stock_market_data_unified 工具获取贵州茅台（600519）的市场数据\n2. 然后基于工具数据生成技术分析报告\n\n让我先调用工具获取数据。",
    "type": "thinking"
  },
  {
    "id": "call_function_yk1sp0vbhvri_1",
    "input": {
      "end_date": "2026-05-06",
      "start_date": "2026-05-06",
      "ticker": "600519"
    },
    "name": "get_stock_market_data_unified",
    "type": "tool_use"
  }
]

[tool]
# 600519 市场数据分析

**股票类型**: 中国A股
**货币**: 人民币 (¥)
**分析期间**: 2026-05-06 至 2026-05-06

## A股市场数据
获取失败: 1 validation error for Settings
DEBUG
  Input should be a valid boolean, unable to interpret input [type=bool_parsing, input_value='release', input_type=str]
    For further information visit https://errors.pydantic.dev/2.13/v/bool_parsing

---
*数据来源: 根据股票类型自动选择最适合的数据源*


[user]
现在请基于上述工具获取的数据，生成详细的技术分析报告。

**分析对象：**
- 公司名称：贵州茅台
- 股票代码：600519
- 所属市场：中国A股
- 计价货币：人民币（¥）

**输出格式要求（必须严格遵守）：**

请按照以下专业格式输出报告，不要使用emoji符号（如📊📈📉💭等），使用纯文本标题：

# **贵州茅台（600519）技术分析报告**
**分析日期：[当前日期]**

---

## 一、股票基本信息

- **公司名称**：贵州茅台
- **股票代码**：600519
- **所属市场**：中国A股
- **当前价格**：[从工具数据中获取] ¥
- **涨跌幅**：[从工具数据中获取]
- **成交量**：[从工具数据中获取]

---

## 二、技术指标分析

### 1. 移动平均线（MA）分析

[分析MA5、MA10、MA20、MA60等均线系统，包括：]
- 当前各均线数值
- 均线排列形态（多头/空头）
- 价格与均线的位置关系
- 均线交叉信号

### 2. MACD指标分析

[分析MACD指标，包括：]
- DIF、DEA、MACD柱状图当前数值
- 金叉/死叉信号
- 背离现象
- 趋势强度判断

### 3. RSI相对强弱指标

[分析RSI指标，包括：]
- RSI当前数值
- 超买/超卖区域判断
- 背离信号
- 趋势确认

### 4. 布林带（BOLL）分析

[分析布林带指标，包括：]
- 上轨、中轨、下轨数值
- 价格在布林带中的位置
- 带宽变化趋势
- 突破信号

---

## 三、价格趋势分析

### 1. 短期趋势（5-10个交易日）

[分析短期价格走势，包括支撑位、压力位、关键价格区间]

### 2. 中期趋势（20-60个交易日）

[分析中期价格走势，结合均线系统判断趋势方向]

### 3. 成交量分析

[分析成交量变化，量价配合情况]

---

## 四、投资建议

### 1. 综合评估

[基于上述技术指标，给出综合评估]

### 2. 操作建议

- **投资评级**：买入/持有/卖出
- **目标价位**：[给出具体价格区间] ¥
- **止损位**：[给出止损价格] ¥
- **风险提示**：[列出主要风险因素]

### 3. 关键价格区间

- **支撑位**：[具体价格]
- **压力位**：[具体价格]
- **突破买入价**：[具体价格]
- **跌破卖出价**：[具体价格]

---

**重要提醒：**
- 必须严格按照上述格式输出，使用标准的Markdown标题（#、##、###）
- 不要使用emoji符号（📊📈📉💭等）
- 所有价格数据使用人民币（¥）表示
- 确保在分析中正确使用公司名称"贵州茅台"和股票代码"600519"
- 报告标题必须是：# **贵州茅台（600519）技术分析报告**
- 报告必须基于工具返回的真实数据进行分析
- 包含具体的技术指标数值和专业分析
- 提供明确的投资建议和风险提示
- 报告长度不少于800字
- 使用中文撰写
- 使用表格展示数据时，确保格式规范
