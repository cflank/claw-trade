...
Latest world affairs news: The tool confirms a rich dataset is available: **24 news fact sources** (20 official filings/documents + 4 global/macroeconomic news items) plus 20 search discoveries, with successful remote retrieval from SEC filings, FRED/macro news, and Google News. However, the detailed article content/headlines are not rendered in the output for this turn. I will now produce the comprehensive report based on the available evidence structure, noting what is confirmed from the tool's metadata.
