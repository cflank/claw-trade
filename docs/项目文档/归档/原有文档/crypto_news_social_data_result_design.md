...
- `claw_request_data` 已实现官方公告/RSS/页面、GitHub releases、交易所公告配置源、监管 feed、DefiLlama 安全/融资背景、Polymarket 事件预期和商业搜索发现。
- `claw_request_data` 已实现 Alternative.me、LunarCrush、X、Reddit、Telegram、Discord、Polymarket 和商业搜索发现。
...
- Scrapling 暂不接入；当前数据结果没有 Scrapling provider、工具依赖或 stage 依赖。未来如需接入，只能作为指定公开页面提取工具另行评审，不能当新闻源或舆情源。
- 不新增 runtime gate / guard；数据结果只输出数据质量材料，stage 是否打开仍由人类批准和现有 stage policy 控制。

最新人类决策是：正式 12 个 CRYPTO TradingAgents worker 和 `report_polisher` 全部打开。`news_analyst` 和 `social_analyst` 必须通过数据结果调用拿材料。若数据结果不可见、未调用成功、provider 缺 key、provider 失败或返回 `partial` / `insufficient`，worker 的 L1 报告必须直接说明资料缺口和后续取数需求，不能补写真实新闻事实、真实社交舆情结论或完整覆盖声明。`report_polisher` 只整理已批准上游报告，不补写缺失新闻、舆情或投资判断。
...
- 通用搜索 API、Polymarket、Alternative.me 都不直接暴露给 worker；Scrapling 当前不接入；worker 只看到封装后的数据结果工具。

## 3. 工具范围

### 3.1 `claw_request_data`
...
| Scrapling | 暂不接入；不作为当前新闻或舆情数据结果依赖 |
...
### 5.2 `claw_request_data` 输出

```json
{
  "schema_version": "claw_request_data.v1",
  ...
  "readiness": {
    "status": "partial",
    "reason": "official/regulatory sources available; social sources not part of this data result"
  },
  "reader_brief": ""
}
```
...
- 追加 `data_gaps`。
- 数据结果可以是 `partial`，但不能把该 provider 视为成功。
...
- 已注册并挂载 `claw_request_data`。
- 不把搜索 API、Polymarket 或 Alternative.me 直接暴露给 worker；Scrapling 当前不接入。
- worker 必须先调用数据结果；若数据结果不可见、调用失败、缺 key、返回空或 `insufficient`，必须输出新闻资料缺口报告。

验证条件：

- `claw_request_data` 工具已注册。
- OpenClaw provider payload 证明 `news_analyst` 当前 turn 只看到 `claw_request_data`。
...
- 已注册并挂载 `claw_request_data`。
- 不把搜索 API、Polymarket 或 Alternative.me 直接暴露给 worker；Scrapling 当前不接入。
- worker 必须先调用数据结果；若数据结果不可见、调用失败、缺 key、返回空或 `insufficient`，必须输出舆情资料缺口报告。

验证条件：

- `claw_request_data` 工具已注册。
- OpenClaw provider payload 证明 `social_analyst` 当前 turn 只看到 `claw_request_data`。
...
- 如果 social 数据结果缺少某个平台 payload，下游必须把它当作有限材料，不能补写 Reddit/X/Telegram/Discord 舆情。
...
- 已注册 `claw_request_data` 与 `claw_request_data`，并挂到 CRYPTO `news_analyst` / `social_analyst`。
...
- OpenClaw provider payload 证明 worker 实际只看到对应数据结果工具。
