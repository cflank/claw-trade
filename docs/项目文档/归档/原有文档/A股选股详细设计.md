...
| 收盘后确定性批处理生成 top 20 candidate cache | 2、4、5、6 | 后台 data run 状态机、函数级 job、candidate cache contract；策略全集和 v1 跨策略排序权重已按 2026-05-28 口径确认 | 否；provider adapter 与字段覆盖实测仍见 §0.2 |
...
| strategist/skeptic 第一版可见 candidate cache tool | 7、8、13 | stage policy 和 provider payload 验收严格等于 `claw_get_selection_candidate_cache` | 否 |
| manager/PM 第一版无工具 | 7、8、13 | `allowed_tools=()`，provider payload 不得有 tool schema | 否 |
| `claw_get_selection_candidate_cache` 只读 approved cache | 6、7、9 | runtime context 锁定 run；只读 OpenViking approved material；不带业务参数 | 否 |
...
| candidate cache 未批准、过期或 hash/readback 不一致不得进入 prompt | 4、6、7、9、12 | approval/readback/hash/manifest/lineage gate；未批准或过期不启动 worker，可触发后台 refresh；hash/readback/lineage 损坏 fail closed | 否 |
| Python 确定性层不能写自然语言入选理由或投资判断 | 1、2、5、6、12 | candidate cache 只含事实表、字段说明、数据质量、来源摘要；理由由 worker L1 产出 | 否 |
...
| candidate cache 摘要可给 manager/PM，但不得含 raw/debug/protocol/ref/hash 文本 | 3、6、8、13 | `CandidateCacheSummary` 单独模型；manifest/hash/lineage 仅审计可见 | 否 |
| 数据源复用 data_gateway、Mongo、OpenViking | 2、5、11、12 | selection DataNeed audit 复用 provider attempt/cache/normalized/gap/readiness 语义 | 具体 provider plugin/adapter 实测需确认 |
| 东财不作为默认稳定依赖，不允许隐藏 fallback provider | 11、13、15 | provider 来源从 DataNeed planner / ProviderCallSpec 证据取；失败写 attempt/gap，不切旧路径 | 具体 provider matrix 需实测 |
| myhhub/stock 与 Sequoia-X 策略全部纳入第一版优先实现 | 5、6、11、13 | 每条策略以 source + variant 形式进入 approved strategy config；缺字段必须补数据合同，不得静默裁剪 | 否 |
| v1 透明排序权重已确认 | 5、6、13 | 基础分 100 + 风险/数据缺口扣分；权重必须进入 strategy config 与 candidate cache 审计 | 否 |
| selection 远端刷新不伪装成单 ticker `旧历史请求计划` | 3、5、11 | selection data job 生成 `DataNeed`，由统一 planner 生成 `ProviderCallSpec` | 不扩展旧 `旧历史请求计划` 生成远端请求清单 |
...
| Artifact approval 负责候选缓存与 worker L1 入下游 | 2、6、8、12 | candidate cache approval、worker artifact approval、manifest/hash/readback | 否 |
...
|---|---|---|---|
| 事件类 provider 覆盖 | 影响定向增发事件策略资料质量 | 策略必须保留；若真实事件源不可用，candidate cache 记录本轮事件资料缺口，不得伪造命中 | 否 |
...
- 不把 `/select` workflow、A股策略、candidate cache 生成、用户确认或 report handoff 写入 OpenClaw。
- 不让 `/select` 现场拉全市场数据。
...
|---|---|---|---|---|---|---|---|
| selection request model | `src/claw_trade/selection/models.py`（新增） | 定义 `SelectRequest`、run、candidate cache、dispatch、decision、confirmation DTO | `/select` 命令解析结果、latest run refs | typed DTO | 不复用 ticker-centric `RunRequest` 承担 selection 语义 | `src/claw_trade/data_gateway/models.py` 的 `Market` | 无效市场/profile/date 返回 typed error |
...
| select command controller | `src/claw_trade/selection/controller.py`（新增） | 检查可用 completed+approved cache；可用才调度 4 worker、产出结果；不可用且属于 no-run/no_candidate/stale/candidate_cache_not_approved/warehouse 证据不足时触发后台 refresh | `SelectRequest` | `SelectionWorkflowRun` / reader result / refresh 状态 | 不直接调 provider、不直接读表、不同步跑全市场、不打分、不写理由 | selection store、scheduler/refresh gateway、OpenViking、OpenClaw client | 返回“补数已启动/已有补数在跑/补数通道未配置”或明确 failed 状态 |
...
| candidate cache builder | `src/claw_trade/selection/candidate_cache.py`（新增） | 生成 worker 可读事实表和审计 manifest | top20、features、hits、quality | `candidate_cache.md/json`、manifest | 不写“值得买/看好”等理由 | engine outputs、source refs | candidate cache 字段缺失则 approval failed |
| artifact approval | `src/claw_trade/selection/artifacts.py`（新增）或扩展 `src/claw_trade/artifacts/**` | 校验 candidate cache 与 selection worker L1，写 OpenViking approved material | artifact body、manifest、hash | `CandidateCacheRef`、`SelectionWorkerArtifact` | 不把 failed artifact 放入下游 | OpenViking client、hash/readback | failed 状态，不继续 |
| candidate cache tool backend | `src/claw_trade/selection/tools.py`（新增） | 实现 `claw_get_selection_candidate_cache` 后端 | runtime context | model-visible cache body | 不调 provider/raw/score | selection store、OpenViking | 返回明确 tool error code |
...
| `trade_date` | `str` | 必填 | trading calendar | 可出现在 candidate cache meta | 是 | 已收盘交易日 | 盘中日期 |
| `lookback_trading_days` | `int` | 必填 | approved config | 否 | 是 | 总体设计默认建议 260，但值需批准 | 硬编码未批准值 |
| `universe_scope` | `str` | 必填 | approved config | 可在 candidate cache meta | 是 | 全 A 或批准股票池 | 手工补股票 |
| `data_need_audit_ref` | `str` | 必填 | selection data job / DataNeed audit | 否 | 是 | batch-scope，不伪装 ticker，不携带 provider 源清单 | raw payload |
...
| `candidate_cache_ref` | `CandidateCacheRef | None` | 可选 | approval | 否 | 是 | completed 必填且 approved；no_candidate 必须为空 | 未批准 artifact |
...
| `candidate_cache_ref` | `CandidateCacheRef` | 必填 | validation | 否 | 是 | approved/hash/readback valid | raw refs |
...
| `cache_summary_ref` | `str` | 必填 | candidate cache builder | 否 | 是 | manager/PM 只读 summary body | raw refs in prompt |

### 3.6 `CandidateCacheManifest`

| 字段名 | 类型 | 必填/可选 | 来源 | 模型可见 | 审计可见 | 约束 | 不能包含什么 |
|---|---|---|---|---:|---:|---|---|
| `schema_version` | `str` | 必填 | candidate cache builder | 否 | 是 | semver/date version | - |
| `selection_run_id` | `str` | 必填 | candidate cache builder | 否 | 是 | run 绑定 | - |
| `market` / `profile` / `trade_date` | `str` | 必填 | run plan | 否 | 是 | CN_A only | fallback market |
| `candidate_count` | `int` | 必填 | candidate cache builder | 可见摘要可含 | 是 | approved cache 为 1..20；20 是评审上限，不是必须凑满；0 只可进入 no-candidate 状态，不生成 fake approved cache | top20 以外明细 |
...
| `cache_body_sha256` | `str` | 必填 | approval | 否 | 是 | readback hash | prompt 正文 |
...
### 3.7 `CandidateCacheSummary`

| 字段名 | 类型 | 必填/可选 | 来源 | 模型可见 | 审计可见 | 约束 | 不能包含什么 |
|---|---|---|---|---:|---:|---|---|
| `summary_md` | `str` | 必填 | candidate cache builder | 是 | 是 | top20 排序事实表、字段说明、数据质量、读者化来源名称/日期 | refs/hash/manifest/raw/debug/protocol |
| `candidates` | `tuple[CandidateFactRow, ...]` | 必填 | candidate cache builder | 是 | 是 | 每行只含事实字段 | 自然语言入选理由 |
| `data_quality_summary` | `str` | 必填 | candidate cache builder | 是 | 是 | 缺口与时效说明 | provider attempt JSON |
| `source_summary` | `str` | 必填 | candidate cache builder | 是 | 是 | 读者化来源摘要 | source secret/headers |
...
| `enter_report` | `tuple[DecisionTicker, ...]` | 必填 | PM L1 解析/approval | reader 可见 | 是 | 0-3 只，且来自 candidate cache | 买入/持有/卖出 |
| `watch` | `tuple[DecisionTicker, ...]` | 必填 | PM L1 | reader 可见 | 是 | 来自 candidate cache | 交易建议 |
| `reject` | `tuple[DecisionTicker, ...]` | 必填 | PM L1 | reader 可见 | 是 | 来自 candidate cache | 目标价/止损价 |
...
| `company_name` | `str` | 必填 | candidate cache/identity resolver | 否 | 是 | 不猜公司名；缺则 resolver | 选股理由 |
...
| `company_name` | `str` | 必填 | PM 明确输出/候选池确定性映射 | 是 | 是 | 与 ticker 一致；只能从 candidate cache 映射补齐，不允许语义补全 | 臆测公司名 |
...
| `domain` | `str` | 必填 | candidate cache domain | 否 | 是 | 属于已批准 selection 域 | 未批准域 |
...
### 3.16 `SelectionDataNeedAudit`

| 字段名 | 类型 | 必填/可选 | 来源 | 模型可见 | 审计可见 | 约束 | 不能包含什么 |
|---|---|---|---|---:|---:|---|---|
| `audit_id` | `str` | 必填 | data job/store | 否 | 是 | 唯一 id | - |
| `scope` | `Literal["selection_batch"]` | 必填 | data job | 否 | 是 | 固定为 `selection_batch`，不得伪装单 ticker plan | `/report` ticker scope |
...
| `coverage_groups` | `tuple[str, ...]` | 必填 | data job | 否 | 是 | 只说明本轮需要覆盖哪些数据组 | 当成 provider 硬编码限制 |
| `data_need_refs` | `tuple[str, ...]` | 可选 | DataNeed planner evidence | 否 | 是 | 远端补数只能追溯到 DataNeed/ProviderCallSpec | provider 源清单 |
| `ttl_policy_ref` | `str` | 必填 | approved policy | 否 | 是 | 与 freshness 规则绑定 | 随机 TTL |
| `lineage_root_ref` | `str` | 必填 | data job | 否 | 是 | 用于 cache/run/audit 追溯 | 无 lineage |
...
| `fetching_data` | DataNeed refresh finished | `normalizing_inputs` | `failed` | 是，新 run id | 否 |
...
| `filtering_and_scoring` | approved strategy config used | `building_candidate_cache`；若真实候选为 0 则 `no_candidate` | `failed` | 是，新 run id | 否 |
| `building_candidate_cache` | cache body/manifest written | `approving_candidate_cache` | `failed` | 是，新 run id | 否 |
| `approving_candidate_cache` | readback/hash/manifest verified | `completed` | `failed` | 是，新 run id | 否 |
...
def run_selection_data_job(plan):
    ...
    try:
        ...
        mark_status(plan.run_id, "building_candidate_cache")
        candidate_cache = build_candidate_cache(plan, scored)
        mark_status(plan.run_id, "approving_candidate_cache")
        approved_ref = approve_candidate_cache(plan, candidate_cache)
...
candidate_cache_lineage_incomplete
...
| `loading_completed_selection_run` | latest found | `validating_candidate_cache` | `no_completed_selection_run` / `no_candidate_selection_run` / `stale_selection_run`，并尝试触发后台 refresh 返回 started/running/unconfigured | 是 | 否 |
| `validating_candidate_cache` | candidate cache verified | `select_run_created` | candidate cache failure states | 是，after new data run | 否 |
...
### 4.3 candidate cache approval 状态机

状态枚举建议名：`CandidateCacheApprovalStatus`。
...
| 当前状态 | 触发事件 | 下一状态 | 失败状态 | 允许重试 | 允许继续 |
|---|---|---|---|---:|---:|
| `draft` | candidate cache builder submits | `submitted` | `rejected` | 是，重建 candidate cache | 否 |
| `submitted` | schema check | `schema_validating` | `rejected` | 是，重建 candidate cache | 否 |
...
```python
def approve_candidate_cache(candidate_cache):
    validate_candidate_cache_schema(candidate_cache.model_visible_body)
    validate_no_forbidden_prompt_material(candidate_cache.model_visible_body)
    validate_lineage(candidate_cache.manifest.source_lineage_refs)
    receipt = openviking.write_material(candidate_cache.model_visible_body)
    body = openviking.readback(receipt)
    if sha256(body) != candidate_cache.manifest.cache_body_sha256:
        return integrity_failed("candidate_cache_hash_mismatch")
    return CandidateCacheRef(..., approved_at=now(), expires_at=ttl_for_trade_date(...))
```
...
def confirm_selection_ticker(request):
    ...
    # 再校验：确认时必须重新验证 candidate cache freshness + readback/hash/manifest/lineage 一致性
    recheck = validate_candidate_cache_for_confirmation(workflow.candidate_cache_ref, now=clock.now())
    if not recheck.ok:
...
2026-05-28 人类确认：`myhhub/stock` 与 `Sequoia-X` 审计到的本地可复现策略全部纳入第一版优先实现。实现时不得因为当前字段或 provider 未接好而裁剪策略；缺字段必须补 data_gateway 字段合同和 provider plugin/adapter。运行时不得伪造命中，真实字段不可得时应在 candidate cache 中记录该策略本轮资料缺口。
...
- `stable_top_n(scores, n=20)` 表示最多取 20 只给 selection workers 评审；真实候选不足 20 时不得补齐，真实候选为 0 时不得生成 fake approved candidate cache。
- PM 最终进入 `/report` 的数量仍是 0..3；这和 candidate cache 的最多 20 只不是同一层。
...
- candidate cache 模型可见正文可以展示分项事实和读者化字段说明，但不得写 Python 生成的自然语言入选观点。
...
- 输出：selection run plan / DataNeed audit 输入。
...
def build_selection_run_plan(...):
    ...
    if strategy_config is None:
        raise SelectionJobError("strategy_config_unapproved")
    data_need_audit_ref = selection_data_job.create_data_need_audit(...)
    return SelectionRunPlan(..., data_need_audit_ref=data_need_audit_ref)
```

### 5.4 `fetch_selection_market_data(...)`

- signature: `fetch_selection_market_data(plan: SelectionRunPlan) -> SelectionMarketDataRefs`
- 输入：selection data need audit。
...
- failure modes：data need audit missing、evidence write failed、credential/rate/license gaps、data authenticity failure。

```python
def fetch_selection_market_data(plan):
    audit = selection_data_need_audit_store.load(plan.data_need_audit_ref)
    assert audit.scope == "selection_batch"
    refs = data_gateway.fetch_data_needs(audit)
...
### 5.9 `build_candidate_cache(...)`

- signature: `build_candidate_cache(plan: SelectionRunPlan, scores_ref: CandidateScoresRef) -> CandidateCacheDraft`
- 输入：最多 20 只候选、features、hits、data quality、source summaries。
- 输出：draft cache body + manifest。
...
```python
def build_candidate_cache(plan, scores_ref):
    data = score_store.read(scores_ref)
    if len(data.top_candidates) > 20:
        raise SelectionJobError("candidate_cache_top_limit_exceeded")
    body = render_fact_table_only(data)
    validate_no_python_opinion_language(body)
    manifest = build_candidate_cache_manifest(plan, data)
    return CandidateCacheDraft(body_md=body, body_json=data.model_visible_json, manifest=manifest)
```

### 5.10 `approve_candidate_cache(...)`

- signature: `approve_candidate_cache(plan: SelectionRunPlan, draft: CandidateCacheDraft) -> CandidateCacheRef`
- 输入：draft candidate cache。
- 输出：approved candidate cache ref。
...
```python
def approve_candidate_cache(plan, draft):
    validate_candidate_cache_contract(draft)
    receipt = openviking.write_material(target=selection_candidate_cache_target(plan), content=draft.body_md)
    readback = openviking.read_for_receipt_verification(receipt)
    if sha256(readback) != sha256(draft.body_md):
        raise SelectionJobError("candidate_cache_integrity_failed")
    return store.write_candidate_cache_ref(plan.selection_run_id, receipt, draft.manifest)
```

### 5.11 `mark_selection_run_completed(...)`

- signature: `mark_selection_run_completed(selection_run_id: str, candidate_cache_ref: CandidateCacheRef) -> SelectionDataRun`
- 输入：run id + approved cache ref。
- 输出：completed data run。
- side effects：status update, latest pointer update if freshness rules allow。
- failure modes：candidate cache not approved, immutable run overwrite attempt。

```python
def mark_selection_run_completed(run_id, candidate_cache_ref):
    assert candidate_cache_ref.approved_at
    run = store.load_data_run(run_id)
    if run.status in TERMINAL_STATUSES:
        raise SelectionJobError("immutable_run_update_forbidden")
    completed = store.transition_to_completed(run_id, candidate_cache_ref)
...
`candidate_cache.md` 的模型可见正文建议结构：
...
```json
{
  "schema_version": "selection_candidate_cache.v1",
  "selection_run_id": "...",
  "market": "CN_A",
...
- readback body sha256 必须等于 manifest `cache_body_sha256`。
- readback size 必须等于 receipt size。
- readback 失败或 hash mismatch 时状态为 `candidate_cache_integrity_failed`，不得启动 worker。
...
`claw_get_selection_candidate_cache` 只读取 approved cache；`/select` controller 只有在进入 worker 前才读取并绑定：

- status=`completed` 的 data run。
- approved candidate cache。
- readback/hash/manifest/lineage 全部通过的 candidate cache。
- market/profile 与 request 匹配的 candidate cache。
...
## 7. `claw_get_selection_candidate_cache` 工具详细设计

### 7.1 tool intent

- intent 名：`selection_candidate_cache`（建议名）。
- canonical provider-visible tool name：`claw_get_selection_candidate_cache`。
- 作用：让 `selection_strategist` 和 `selection_skeptic` 在当前 OpenClaw turn 内读取本轮 approved top20 candidate cache。
...
`selection_skeptic` 同样使用 `selection_candidate_cache`。
...
- `selection_run_id`。
- `candidate_cache_material_id` 或受控 ref。
- `worker_id`。
- `stage`。
...
```python
def claw_get_selection_candidate_cache(ctx):
    ...
    cache_ref = run.candidate_cache_ref
    validate_cache_ref_matches_context(cache_ref, command)
    validate_candidate_cache_not_stale(cache_ref)
    body = openviking.readback(cache_ref.l1_uri)
    validate_sha256(body, cache_ref.content_sha256)
...
| `candidate_cache_not_approved` | candidate cache 未 approved | tool hard fail |
| `candidate_cache_stale` | candidate cache 过期 | tool hard fail |
| `candidate_cache_integrity_failed` | readback/hash/manifest/lineage 不一致 | tool hard fail |
| `candidate_cache_lineage_incomplete` | provider/feature lineage 缺失 | tool hard fail |
| `candidate_cache_forbidden_material` | body 含 raw/debug/protocol | tool hard fail |
...
```text
selection_strategist tools == ["claw_get_selection_candidate_cache"]
selection_skeptic tools == ["claw_get_selection_candidate_cache"]
...
- strategist/skeptic 首轮 prompt 不预注入完整 candidate cache；候选材料来自工具调用结果。
- manager/PM prompt 只含上游 approved L1 和受控 `candidate_cache_summary`。
...
- model-visible materials：运行上下文；工具调用后的 candidate cache
...
- approval gate：L1/readback/hash/provider payload/tool calls 全部通过；输出不得新增 candidate cache 之外股票
- downstream handoff：approved L1 给 skeptic、manager、PM
- provider payload 必验项：工具严格等于 candidate cache tool；首轮 prompt 未包含完整 candidate cache 正文
...
- allowed_tools：`("claw_get_selection_candidate_cache",)`
- prompt variables：`market`、`profile`、`trade_date`、`selection_run_id`、`select_workflow_run_id`
- model-visible materials：candidate cache 工具结果、approved strategist L1
...
- provider payload 必验项：工具严格等于 candidate cache tool；上游材料是 approved strategist L1 正文
...
- model-visible materials：approved strategy review、approved skeptic review、受控 `candidate_cache_summary`
...
- allowed_tools：`()`
- prompt variables：`market`、`profile`、`trade_date`、`select_workflow_run_id`
- model-visible materials：approved ranked watchlist、strategy review、skeptic review、受控 `candidate_cache_summary`
...
- approval gate：最终结论只能是进入 `/report`、观察、放弃；进入 `/report` 0-3 只且来自 candidate cache
...
def handle_select_command(raw_text, request_id, user_id):
    ...
    cache_ref = latest.candidate_cache_ref
    if cache_ref is None:
        return trigger_selection_data_refresh_or_unavailable(request, reason="candidate_cache_not_approved")

    cache_check = validate_candidate_cache_for_select(cache_ref, request)
    if cache_check.code == "candidate_cache_not_approved":
        return trigger_selection_data_refresh_or_unavailable(request, reason="candidate_cache_not_approved")
    if cache_check.code == "candidate_cache_integrity_failed":
        return unavailable("candidate_cache_integrity_failed")
    if cache_check.code == "candidate_cache_lineage_incomplete":
        return unavailable("candidate_cache_lineage_incomplete")
    if not cache_check.ok:
        return unavailable(cache_check.code)
    ...
    validate_provider_payload(
        strategist_result,
        expected_tools=("claw_get_selection_candidate_cache",),
    ...
    summary = load_model_visible_candidate_cache_summary(cache_ref)
...
- `no_completed_selection_run`、`no_candidate_selection_run`、`stale_selection_run`、`candidate_cache_not_approved`、warehouse 证据不足不启动任何 worker，只触发后台 refresh/job 并返回 started/running/unconfigured。
- `candidate_cache_integrity_failed`、`candidate_cache_lineage_incomplete` 不启动任何 worker；hash/readback/lineage 损坏仍 fail closed，不用补数隐藏问题。
...
- ticker 必须在 candidate cache top20 中。
...
| decision 过期或 candidate cache stale | `selection_decision_stale`，不启动 report |
...
- `/report` 远端取数由 worker 通过 `claw_request_data` 提交业务数据项和用途；`DataNeed` 只在数据层内部生成。
- selection batch 远端刷新由 data job 提交全市场/批量业务数据项请求，再由数据层内部生成 `DataNeed`，不能把 5000 只股票伪装成某个 ticker。

selection data job 审计语义必须满足：

- market/profile/trade_date/lookback_days/universe_scope。
- coverage group：股票列表、日线历史、当日快照、基础估值、行业分类。
- DataNeed id、ProviderCallSpec、attempt id、cache key、normalized ref、失败原因、data gap。
...
- cache receipts。
- data gaps/readiness。
- selection data need audit / ProviderCallSpec lineage。
...
- approved candidate cache material。
...
### 11.4 selection DataNeed 审计与 `/report` 单标的取数的关系

关系是“复用 DataNeed planner、ProviderCallSpec 和 evidence 语义”，不是“复用单标的请求形状”。
...
- 验收必须能从 candidate cache 字段追溯到 selection DataNeed audit、ProviderCallSpec、provider attempts、normalized refs、feature snapshot。
...
- candidate cache approved 后才可被 tool 读取。
...
- run id、stage、worker/candidate_cache target。
...
| normalized result | Mongo/evidence store | 否 | feature/candidate cache builder 输入 |
| feature snapshot | selection evidence store | 否 | scoring/filter audit |
| candidate cache summary/body | OpenViking approved material | 是，受控 | strategist/skeptic tool result，manager/PM summary |
...
- output ticker set is subset of candidate cache where applicable。
...
| `test_candidate_cache_schema_rejects_forbidden_python_reason` | cache body with opinion text | `validate_candidate_cache_contract` | rejected | Python 不写自然语言入选理由 |
| `test_candidate_cache_top_limit` | 21 rows | `build_candidate_cache` | fails `top_limit_exceeded` | top 20 |
...
| `test_selection_tool_registry_mapping` | tool registry | resolve `selection_candidate_cache` | `claw_get_selection_candidate_cache` | tool_names mapping |
...
| `test_selection_candidate_cache_tool_has_no_business_params` | plugin registration | inspect JSON schema | `properties == {}` | 工具无业务参数 |
| `test_selection_tool_backend_import_block` | monkeypatch provider/raw modules | call tool with approved cache | provider/raw modules not imported | tool 不查 provider/raw |
...
| 测试名 | fixture | 执行动作 | 断言 | 证明哪个总体设计要求 |
|---|---|---|---|---|
| `test_selection_data_job_generates_approved_candidate_cache` | small CN_A normalized fixture + approved config | run background job | completed run + approved cache + lineage | 后台确定性闭环 |
...
| `test_select_no_candidate_run_triggers_refresh_without_old_completed` | latest no-candidate + older completed + provider fetch spy + refresh gateway spy | `/select` | refresh started/running/unconfigured；provider spy not called | 0 候选不造 candidate cache、不回退旧 run，触发后台补数 |
...
| `test_select_unapproved_candidate_cache_triggers_refresh_before_worker` | completed run without approved cache | `/select` | `candidate_cache_not_approved` + refresh started/running/unconfigured；no OpenClaw call | approved-only；不使用半成品 |
| `test_select_hash_mismatch_stops_before_worker` | candidate cache readback mismatch | `/select` | `candidate_cache_integrity_failed` | hash/readback gate |
| `test_selection_confirmation_stale_or_integrity_mismatch_does_not_start_report` | waiting workflow + stale/hash mismatch candidate cache | confirm | `selection_decision_stale`/integrity error；report queue unchanged | 确认再校验门 |
| `test_selection_full_workflow_dispatch_order` | approved cache + real OpenClaw focused run evidence（或纯状态机 fixture，且标注“非 runtime/live 验收”） | `/select` | 4 dispatches fixed order | LLM 不决定调度 |
| `test_selection_tool_reads_same_candidate_cache_for_strategist_and_skeptic` | approved cache | run both worker turns | same candidate cache hash/readback | same approved candidate cache |
...
| 测试名 | fixture | 执行动作 | 断言 | 证明哪个总体设计要求 |
|---|---|---|---|---|
| `test_selection_strategist_provider_payload_tools` | fresh OpenClaw run | inspect payload | only `claw_get_selection_candidate_cache` | provider payload final evidence |
| `test_selection_skeptic_provider_payload_tools` | fresh OpenClaw run | inspect payload | only `claw_get_selection_candidate_cache` | tool matrix |
...
| `test_selection_payload_candidate_cache_tool_result_boundary` | sequence=2 provider requests | inspect messages | tool result has cache body, no raw/debug/protocol | prompt boundary |
...
| `test_selection_lineage_incomplete_fails` | candidate cache without feature/provider lineage | `/select` | `candidate_cache_lineage_incomplete` | lineage required |
...
| `test_selection_tool_never_calls_removed_data_gateway` | approved cache + data_gateway/provider spy raising | call tool | tool succeeds from approved cache; data_gateway/provider spy not called | tool only reads approved cache |
...
- candidate cache approved material readback/hash.
...
### 14.2 candidate cache artifact contract

- 修改文件：`src/claw_trade/selection/candidate_cache.py`、`src/claw_trade/selection/artifacts.py`。
- 新增函数：`build_candidate_cache`、`validate_candidate_cache_contract`、`approve_candidate_cache`。
- 验收测试：`test_candidate_cache_schema_rejects_forbidden_python_reason`、`test_candidate_cache_top_limit`。
...
- 新增函数：§5 全部后台函数。
- 验收测试：`test_selection_data_job_generates_approved_candidate_cache`。
...
- 新增函数：`get_selection_candidate_cache_from_context`、tool wrapper registration。
- 验收测试：`test_selection_tool_registry_mapping`、`test_selection_candidate_cache_tool_has_no_business_params`。
...
2. selection data job 的审计模型细节仍可调整，但远端请求来源已冻结为业务数据项请求；`DataNeed -> planner -> ProviderCallSpec` 只在数据层内部发生，不得扩展旧 `旧历史请求计划` 生成历史请求清单。
