...
OpenBB 本体已删除，不是目标运行时依赖。当前只继承它曾经参考过的思想：provider 插件、fetcher、统一字段。
...
这层参考 OpenBB 的 provider/fetcher 思想。
...
- OpenBB 全量 MCP。
- OpenBB Workspace / Excel / CLI。
- OpenBB user_settings 作为主配置。
- OBBject 作为仓库数据结构。
- OpenBB runtime 作为唯一执行核心。
