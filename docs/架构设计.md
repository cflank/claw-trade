...
> **2026-06-13 数据层口径**
>
> 本文保留控制面、OpenClaw、OpenViking、Mongo 等架构边界。数据层运行时口径以 `docs/限流重组.md` 为准：report/frontline worker 通过 `claw_request_data` 提交业务数据项请求，不提交 `DataNeed`、`api_id`、`内部需求类型字段`、历史数据入口或直接 `DataRequest(data_type, fields)` 清单；`DataNeed` 和 `ProviderCallSpec` 只属于数据层内部。

本文是当前架构入口：它归并当前控制面边界、运行时分工、数据层布局和部署口径。已删除的 legacy 文档不再作为产品路径或实施依据。
...
- 数据层负责接收 worker 的业务数据项请求，由内部 planner 生成 provider call，查询仓库、必要时调用 provider、入库证据，并返回数据、证据和缺口。
...
数据层新方向：已删除数据网关 本体不是必须依赖；保留的是 provider 插件、fetcher 流程、统一字段和能力声明思想。目标表名、运行时模块和证据链不再按 `removed_data_gateway_*` 命名。
...
flowchart TB
  ...
  subgraph MaterialPlane["材料层 / 证据展示层"]
    MaterialTools["Material / Data Result Tools\n把数据转成 worker 可读材料"]
    OpenViking["OpenViking\napproved L1/L2 / receipts / readback"]
  end

  subgraph DataLayer["数据层"]
    DataAPI["Data API\n业务数据项 -> DataNeed -> ProviderCallSpec -> DataResult"]
    Warehouse["Warehouse\n统一数据仓库"]
...
flowchart LR
  ...
  subgraph DataModule["数据层模块"]
    Interface["接口层\nDataNeed / ProviderCallSpec / DataResult"]
...
sequenceDiagram
  ...
  participant PS as NeedResolver
  participant PR as ProviderRegistry
  participant BP as ProviderCallScheduler
  ...
  Tool->>API: request_data(item/purpose/instrument/market)
  API->>DS: validate / plan ProviderCallSpec
  ...
  alt warehouse enough
    ...
    DS->>PS: resolve DataNeed to ProviderCallSpec
    PS->>PR: read official catalog / ProviderPlugin endpoints
    PR-->>PS: endpoint facts
    PS-->>DS: planned provider calls
    DS->>BP: coalesce compatible calls
    BP-->>DS: provider call batches
...
User /report
  ...
  -> tool/material layer submits business data items to data layer when it needs external or warehouse data
...
- market：`cn-a-market-data`，通过 `claw_request_data` 按需取数；指标/图表计算在 `src/claw_trade/reports/market_indicators.py` 和 `src/claw_trade/reports/market_charts.py`。
...
- worker 工具可以触发资料收集或材料读写，但数据层本身不生成 worker 数据结果。
- 远端取数入口只接收业务数据项请求，并返回 `DataResult`；`DataNeed`、`ProviderCallSpec` 和 `DataRequest` 只可作为数据层内部对象。
...
- `claw_request_data` 是 CRYPTO `market_analyst` 的唯一市场数据结果入口；工具层向数据层提交业务数据项，数据层负责规划 provider call、外部取数、raw/normalized evidence、cache、attempt 和 data gaps。
- CRYPTO 市场数据结果链路是：`claw_request_data(item) -> DataNeed -> ProviderCallSpec -> DataResult -> normalized crypto data -> CryptoLens analysis engine -> reader_brief`。
- CRYPTO `market_analyst` stage 不直接暴露 `bb_crypto_data__build_trade_context`、CryptoLens raw tool、旧 BB MCP 原子工具或 已删除数据网关 atomic/admin/discovery tools。
- 投资辩论、交易、风险和 PM worker 不得直接调用 CryptoLens、旧 BB MCP、旧 已删除数据网关 原子工具或数据层 provider 插件；它们消费前线 worker 已批准的 L1 自然语言报告。
...
-> 数据层内部 DataNeed
-> 仓库检查
-> planner 读取 official catalog / provider endpoint 事实
-> 生成 ProviderCallSpec
-> 按 endpoint/batch policy 合并可合并调用
-> Scheduler 按 provider bucket + deadline 排队
...
- 限流配额必须来自用户设置；未设置时本地不主动限流，只记录调用计数。
- 支持 `safety_margin`，例如用户设置 1 分钟 10 次时可主动最多发 9 次，或发满 10 次后等待下一窗口。
- 未设置限流而远端返回 429 / quota 类错误时，当前 provider 记为 `rate_limited`，数据层继续尝试下一个可用 provider。
...
CN_A 默认按项目必需数据项走数据层 provider 插件：公告、新闻、行情、财务、资金、筹码都必须绑定真实外部 API，并输出对应标准数据。`domain`、`coverage_group`、`source_role` 只是审计和事实口径标签，不能作为候选硬编码限制；Tushare 仅作为用户配置源，在用户完成配置、验证并启用后，才在能产出同一项目数据项时参与优先级排序。
...
目标态是 `claw_request_data` 内部经数据层 provider 插件获取 CRYPTO 市场资料，写 raw/normalized/attempt 证据，再把 normalized data 交给 CryptoLens 离线分析。它不能把 CoinGlass、Binance、Bybit、FRED、AHR999 等缺失域补写成已覆盖；缺失必须进入 `data_gaps`。

当前 `claw_request_data` / `claw_request_data`（旧实现名 `claw_request_data` / `claw_request_data` 仅作迁移盘点）的 provider 采用可选 key / 可选配置策略：有 credential/config 才调用对应 provider；缺 key、缺 URL 清单、缺频道清单或 provider 失败时不发起假调用，必须在数据结果内记录为 `config_blocked` / `credential_missing` / `error` 或同等 provider attempt，并进入 `data_gaps`。这不是静默 fallback，也不能让新闻或舆情覆盖看起来完整。
...
本文件是当前架构入口。已删除的 legacy 文档不再链接、不再作为运行时或实施依据；需要调整架构时，直接修改当前有效文档和对应测试。
