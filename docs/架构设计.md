...
PM owner 的 profile 边界：

- `CN_A`：最终裁决权威来源是 `portfolio_manager` 的已批准自然语言 L1 report；不因缺少 `pm-decision.json` 阻断。
- 其他已批准 profile（当前主要是 `US`）：可继续使用并校验 `pm-decision.json` 结构化证据。
