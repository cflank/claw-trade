...
frontline 四个 worker 完成后，流程进入 CN/原版投资辩论顺序：`bull_researcher` 先发言，`bear_researcher` 在 prompt 中直接看到 bull 的已批准完整报告正文后反驳；不新增总 gate。
...
- BB 作为 `crypto_market_data_pack` 内部集中读取的市场结构来源，不作为第 13 个 worker，也不作为 worker 可见的独立大 JSON 工具。
- `crypto-trading-analysis` skill 负责加密币指标分析纪律：覆盖指标、解释信号、暴露 `readiness/data_gaps/conflicts/sources`，并提示 worker 不重复拉取已经 ready 的数据域。
- `crypto_market_data_pack` 负责在本地插件内调用 BB/CoinGlass trade context，并把原始 payload 保存为 `provider_raw` 证据；模型可见材料只保留自然语言简报和紧凑摘要。
- `openclaw_plugins/claw-trade-frontline-tools` 注册本地前线资料包工具，包括 CN_A/HK/US 资料包和 CRYPTO `crypto_market_data_pack`、`crypto_fundamental_data_pack`、`crypto_news_data_pack`、`crypto_social_sentiment_pack`。
- CRYPTO `market_analyst` stage 不直接暴露 `bb_crypto_data__build_trade_context`；如未来重新直连 BB 工具，必须重新评审 provider payload 尺寸、材料边界和限流策略。
- `fundamental_analyst` / `news_analyst` 后续如需要 BB，只挂对应有用的数据域，例如链上、宏观、事件、AHR999；不能让 BB 冒充社交情绪来源。
- 投资辩论、交易、风险和 PM worker 默认不直接调用 BB MCP；它们消费前线 worker 已批准的 L1 自然语言报告。
- BB 的 `readiness/data_gaps` 是 worker 可解释的数据质量材料，不是 claw-trade 新 runtime gate。
...
CRYPTO / BB 数据源使用与 `D:\src\BB` 一致的 key 名称，先放在 `.env.local`，未来 UI 设置页应复用同一组字段：
...
当前 BB MCP 实现直接读取这些环境变量。缺 key 时只能记为 provider credential missing 或数据缺口，不能伪装成完整数据。

当前 `crypto_market_data_pack` 内部读取 BB/CoinGlass 市场结构，并使用公开交易所 OHLCV 生成本地技术指标和 PNG 图表资产，默认 Binance public klines，可通过 `CRYPTO_MARKET_*` 变量调整。它不能把 BB/CoinGlass 缺失域补写成已覆盖；缺失必须进入 `data_gaps`。

当前 `crypto_news_data_pack` / `crypto_social_sentiment_pack` 的 provider 采用可选 key / 可选配置策略：有 credential/config 才调用对应 provider；缺 key、缺 URL 清单、缺频道清单或 provider 失败时不发起假调用，必须在资料包内记录为 `config_blocked` / `credential_missing` / `error` 或同等 provider attempt，并进入 `data_gaps`。这不是静默 fallback，也不能让新闻或舆情覆盖看起来完整。
...
`COINGLASS_API_KEY` 只放 `.env.local` 或未来 UI secret store，不写入文档。BB adapter 会在代理根地址后自动拼接 `/v3/api` 或 `/v4/api`；V4 是默认优先版本，V3 只用于老接口。
