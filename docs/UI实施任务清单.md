...
- UI 设置模型生成 `RunRequest`；controller 只读 `RunRequest`；生产 UI 不直接编辑 `.env.local`。
...
|---|---|---:|---|---|---|---|---|
| §0 设计依据与材料范围 | 材料边界、已确认事实、未知点、成功标准 | 1-6 | P1-00, P5-02, P6-03, AR-01 | 实施前检查清单、blocker 表、adversarial review | 文档 lint；任务评审逐项勾选 | 前端框架、传输形态、Channel/PDF/LLM 桥接仍待确认 | 不把待确认能力写成已存在 API |
...
| §2 架构边界 | claw-trade/UI/OpenClaw/数据源/PDF 职责 | 1-6 | P1-02, P2-03, P5-04, P6-01 | boundary adapter、bridge/service 接口 | 架构边界 contract 测试 | OpenClaw 微信 Channel ID/config schema、LLM 专用 API 待确认 | 不显示 OpenClaw、OpenViking、MongoDB、worker id、provider attempt |
| §3 总体架构图 | Browser/backend/OpenClaw/data 模块关系 | 1-6 | P1-03, P2-01, P3-01, P5-01 | 模块目录、依赖图、接口分层 | import boundary 测试 | UI 后端传输形态、前端框架待确认 | 前端只调 claw-trade 产品 API，不直连内部运行时 |
| §4 UI 信息架构 | 三栏、左侧历史、中间上下文、右侧详情 | 1,3 | P1-04, P3-02, P3-05 | 三栏组件、上下文 store、报告阅读器、右侧状态 | 组件测试/E2E 截图 | 前端框架待确认 | 左侧不显示失败/运行/取消；右侧不长期保留失败 |
| §5 视觉设计规范 | token、字体、间距、组件视觉规则 | 1,3,5 | P1-05, P3-03, P5-05 | 设计 token、核心组件样式、报告正文样式 | `pnpm test`, `pnpm build` 或等价组件检查 | 前端框架待确认 | 状态栏不显示 worker id；数据源卡不显示内部路径 |
| §6 核心状态机 | ChatContext/ReportTask/ScheduledReport/PriceAlert/Channel/DataSource/PDF 状态 | 1-6 | P1-06, P2-04, P4-01, P4-02, P5-03, P6-02 | 状态机枚举、迁移函数、状态机测试 | `uv run pytest tests/unit/ui/test_*_state_machine.py` | 运行中取消能力待确认 | 不伪造取消成功，不绕过完整报告队列 |
...
| §8 API / 服务契约 | 普通 UI API、错误码、幂等、权限 | 1-6 | P1-08, P2-05, P3-06, P4-06, P5-07, P6-05 | API contract、OpenAPI/RPC schema、错误映射 | API contract 测试 | UI 后端传输形态待确认 | 所有普通 API 输出必须是 `*ForUser`，含 scheduled/price alert API |
| §9 模块级设计 | AppShell、Chat、Intent、Queue、Workflow、Repository、Settings 等模块 | 1-6 | P1-02, P1-03, P1-04, P1-08, P1-09, P2-01, P2-03, P2-05, P3-01, P3-04, P3-06, P4-02, P4-05, P5-03, P5-04, P5-05, P6-01, P6-06 | 模块实现和测试 | import/API 单测 | 前端目录与后端传输待确认 | 禁止空骨架作为完成 |
| §10 函数级设计和伪码 | 关键函数行为、输入输出、副作用、错误 | 1-6 | P1-06, P1-08, P1-09, P1-10, P2-01, P2-02, P2-03, P2-05, P2-06, P2-07, P2-08, P3-01, P3-02, P3-04, P3-05, P3-06, P4-02, P4-03, P4-05, P5-03, P5-04, P5-05, P5-06, P6-01, P6-02, P6-03, P6-04 | 函数实现、伪码对应测试 | 每个函数至少一个正向和一个红线测试 | OpenClaw/数据源桥接能力待确认 | 不把伪码里的内部字段透给 UI |
| §11 错误处理和用户文案 | 内部错误映射、显示位置、只进日志错误 | 1-6 | P1-10, P1-07, P2-07, P3-03, P5-03, P5-04, P5-06, P6-02 | `ErrorMessageTranslator`、产品错误码表、禁词测试 | `uv run pytest tests/unit/ui/test_error_message_translator.py` | 错误类别需随实际 bridge 扩展 | 禁止原样返回 MongoDB/OpenViking/OpenClaw Gateway/provider attempt/runtime marker |
| §12 安全和隐私 | 密钥掩码、replace-only、`.env.local` 边界、保存 owner | 5 | P1-07, P5-04, P5-05, P5-07, P6-01, P6-02 | secret replace-only 流程、设置边界测试 | `uv run pytest tests/unit/ui/test_settings_security.py` | secret store/配置服务待确认 | 不返回真实 key、credentialRef、本地路径、端口 |
...
| §15 未决问题 | blocker、推荐决策、保守实现 | 1-6 | B-01..B-10 | blocker register、stop/ask gate | blocker 状态评审 | 全部保持待确认，直到有真实接口证据 | blocker 不得伪装成可实现项 |
...
验证：序列化每个普通 API 成功和失败响应后，扫描禁词/禁字段列表；命中即失败。
...
- 目标：即使 UI 后端传输形态未定，也先稳定 service contract。
- 输入/输出：输入为 §8 API 表；输出为传输无关接口和 DTO schema。
- 涉及模型/API/函数：全部 `*ForUser` DTO。
- 文件范围：`src/claw_trade/ui_contracts/**`, `src/claw_trade/ui_backend/**`；前端目录待确认。
- 验收标准：service 层不依赖 HTTP/IPC/WebSocket 细节。
- 验证命令：`uv run pytest tests/contracts/test_ui_api_contracts.py`
- 禁止事项：不得把传输形态待确认变成阻塞全部后端 contract 的理由。
...
- 文件范围：前端 `AppShell/Layout/ReportHistoryRail/ChatMainPanel/RightRail`。
...
- 涉及模型/API/函数：无。
- 文件范围：前端 theme/style 文件。
- 验收标准：报告正文使用中文衬线；UI 控件不按视口缩放字体；任务进度条稳定。
- 验证命令：`pnpm test && pnpm build`；截图 QA 在前端选型后补。
...
### P1-07 基础 DTO 映射与禁词扫描

- 目标：实现通用 DTO mapper 和普通响应禁词测试。
...
- 验收标准：禁词列表全部有失败样本；OpenClaw/Channel/数据源/PDF 错误映射到产品错误码。
...
- 验收标准：`max_debate_rounds`、`max_risk_discuss_rounds`、`frontline_execution_mode`、`default_profile`、`default_market`、`default_currency`、`default_currency_symbol` 全部从确认时冻结的设置快照进入或映射到 `RunRequest`；`max_rounds_hard_limit` 校验在 settings 层执行；生产 UI 不编辑 `.env.local`。
- 验证命令：`uv run pytest tests/unit/ui/test_workflow_bridge.py tests/unit/test_report_workflow_settings.py`
- 禁止事项：不得让 workflow controller 直接读 UI 设置 store 或 `.env.local`。
...
- 验收标准：左侧历史无失败项；右侧 terminal failed 被移除；禁词扫描通过。
- 验证命令：`uv run pytest tests/unit/ui/test_report_failure_visibility.py`
- 禁止事项：不得为方便调试把失败 run 路径显示给用户。

### P2-08 取消能力分层

- 目标：排队任务可立即取消；运行中取消取决于真实 runtime 能力。
- 输入/输出：输入 `cancelReportTask`; 输出 `ReportTaskForUser`。
- 涉及模型/API/函数：`cancelReportTask`, `ReportWorkflowBridge.requestCancel`。
- 文件范围：`report_queue.py`, `workflow_bridge.py`。
- 验收标准：排队任务转 `cancelled`；运行中只在真实 abort 确认后转 `cancelled`，否则显示“已请求取消”。
...
- 输入/输出：输入图表证据索引/导出证据；输出 `ChartEvidenceForUser[]`。
- 涉及模型/API/函数：`getReportChartEvidence`。
- 文件范围：`chart_evidence.py`。
- 验收标准：ready/missing/failed 有用户可读原因；无静态占位图；不返回内部路径。
...
- 验收标准：不返回 `providerChannelId`；连接失败映射为 `NOTIFICATION_UNAVAILABLE`。
- 验证命令：`uv run pytest tests/unit/ui/test_channel_bridge.py`
- 禁止事项：claw-trade 不保存 Channel 凭据，不实现微信协议。
...
- 验收标准：真实密钥不回传、不由 claw-trade 持久化；无专用 API 时只通过隔离桥接用 `config.get/patch` + `models.authStatus`。
...
- 目标：只支持已内置数据源类型；测试通过才启用；密钥 replace-only。
...
- 验收标准：未知 `custom_http` 拒绝；未测试不能启用；响应无真实 key/credentialRef。
- 验证命令：`uv run pytest tests/unit/ui/test_data_source_settings.py`
- 禁止事项：不得做任意未知 HTTP/JSON 数据源接入或字段映射平台。
...
### P5-07 `.env.local` 与生产 UI 边界

- 目标：生产 UI 保存到 DB/配置服务，创建报告时写入 `RunRequest`；不直接编辑 `.env.local`。
...
- 验收标准：dev/CLI 可读 `.env.local`；product/UI 不写 `.env.local`；controller 不直接读 env；UI 设置保存后生成包含 rounds、frontline mode、default profile/market/currency/currency symbol 的快照；rounds 超过 hard limit 时确认失败且不创建任务。
- 验证命令：`uv run pytest tests/unit/ui/test_report_workflow_settings_ui_boundary.py`
- 禁止事项：不得在生产 UI 暴露 `.env.local` 路径或真实值。
...
- 验收标准：只发送 ready PDF；失败不暴露本地路径；文件能力未知时 `FILE_SEND_UNSUPPORTED`。
- 验证命令：`uv run pytest tests/unit/ui/test_report_file_send.py`
- 禁止事项：不得临时生成未保存内容，不得发送 Markdown 作为微信默认格式。

### P6-03 微信入站产品语义事件

- 目标：仅在 OpenClaw 明确提供入站桥接能力后接入 `request_full_report` / `open_report_summary`。
...
- 验收标准：桥接不存在时不注册替代微信 webhook；不实现微信协议。
...
- 验收标准：所有响应禁词扫描通过；无 mock/stub/fake/fallback 作为成功标准。
...
| T-02 | 单元：队列/workflow bridge | 串行、上限 10、手动优先、去重、RunRequest 设置快照 | `uv run pytest tests/unit/ui/test_report_queue.py tests/unit/ui/test_workflow_bridge.py` |
...
| T-05 | 单元：设置 | 密钥掩码、replace-only、数据源测试、LLM/Channel bridge | `uv run pytest tests/unit/ui/test_data_source_settings.py tests/unit/ui/test_llm_settings_bridge.py tests/unit/ui/test_channel_bridge.py` |
| T-06 | 单元：PDF/通知 | PDF 不改 Markdown、通知摘要、文件能力缺失 | `uv run pytest tests/unit/ui/test_pdf_export_service.py tests/unit/ui/test_report_notification_service.py` |
| T-07 | contract：DTO 红线 | 所有普通 API 返回 `*ForUser`，禁词扫描 | `uv run pytest tests/contracts/test_ui_user_dto_redaction.py tests/contracts/test_ui_api_contracts.py` |
| T-08 | 组件/前端 | 三栏、确认卡、右侧任务、报告阅读器、设置页 | `pnpm test && pnpm build` 或选型后的等价命令 |
| T-09 | E2E 非 live | 普通聊天、报告确认、失败不进历史、追问不触发 workflow、定时/提醒/PDF 文件失败 | `uv run pytest tests/e2e/ui/test_report_user_flows.py` |

测试红线：

- live/provider 不作为本清单验证要求；如后续需要 live，必须另按 `AGENTS.md` live preflight。
- 不允许以 mock/stub/fake/fallback 证明 OpenClaw Channel、PDF 文件发送、微信入站、LLM 保存等真实能力。
- 单元测试可以用 fake client 验证“不可用路径”和错误映射，但不能把 fake client 的成功响应当成真实能力已确认。
- 对未知能力只能测试“不可用时产品降级文案和无泄露”，不能测试“成功发送”。

## 10. Blocker / 待确认能力清单

| 编号 | blocker/待确认能力 | 影响 | 保守实现 | 停止条件 |
|---|---|---|---|---|
| B-01 | OpenClaw 微信 Channel ID/config schema | 设置页和发送目标无法写死 | 使用 `wechat_clawbot` 产品概念；后端从 OpenClaw catalog/schema 映射 | 未确认前不得硬编码 `wechat`/`clawbot` provider ID |
| B-02 | 本地 PDF 文件发送能力 | 微信“发送完整报告”可能不可用 | `sendReportFileViaChannel` 返回 `FILE_SEND_UNSUPPORTED` + 可读提示 | 不得模拟成功 message id |
| B-03 | 微信入站产品语义事件 | 微信内回复“报告”无法直达 claw-trade | 不注册微信 webhook；只保留设备界面按钮/提示 | OpenClaw 未提供事件合同前不得实现 |
| B-04 | LLM 配置专用 API/桥接 | 设置页实现复杂度不同 | 先用 `LlmSettingsBridge` 隔离 `config.get/patch/schema` + `models.authStatus` | 不得把 OpenClaw raw config 暴露给 UI |
| B-05 | UI 后端传输形态 | 影响 route/server 部署 | service contract 先传输无关 | 未决定前不得把 HTTP/IPC/WebSocket 细节写死进业务层 |
| B-06 | 前端框架 | 影响目录、测试命令、组件实现 | 默认计划 React/Vite/TypeScript，但作为待确认 | 未确认前不得大规模迁移前端依赖 |
| B-07 | 数据源持久化/secret store | 影响生产设置保存 | 定义 repository/secret 接口；生产不写 `.env.local` | secret store 未定前不得保存明文密钥 |
| B-08 | 完成摘要提取规则 | 影响完成卡质量 | 确定性标题/项目符号摘取；缺字段给查看全文提示 | 不得新增简报 worker 或补写不存在事实 |
| B-09 | 图表证据来源 | 影响右侧图表状态 | 只显示真实 evidence 聚合可证明内容；缺失显示 missing/原因 | 不得用静态占位或“图表正常” |
| B-10 | 运行中取消能力 | 影响取消按钮语义 | 排队可取消；运行中只显示请求取消，等待真实 abort 确认 | 不得伪造 running -> cancelled 成功 |
...
- 是否用 mock/stub/fake/fallback 证明 OpenClaw、Channel、PDF 文件发送、微信入站、LLM 保存、数据源测试成功？
...
- 普通 UI/API 响应是否包含禁词列表任一项？
- 错误文案是否包含内部端口、本地路径、OpenClaw Gateway、OpenViking、MongoDB、worker id、provider attempt、runtime marker？
- scheduled/price alert API 是否直接返回内部对象？

### AR-04 未知 API 被当成已存在

- 是否写死 OpenClaw 微信 Channel ID/config schema？
- 是否假设本地 PDF 文件发送已经存在？
- 是否假设微信入站事件已经存在？
- 是否假设 LLM 专用保存/测试 API 已存在？
...
| NT-09 | 普通 UI 内部诊断泄露 | 禁词扫描覆盖所有普通 API 成功/失败响应 |
