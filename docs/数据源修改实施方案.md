...
- OpenBB 当前版本是否能承载 extension/MCP pack endpoint：若不能承载 `get_market_pack/get_fundamental_pack/get_news_pack/get_social_pack`，必须停止重新设计，不能退回本地 Python data_gateway 直连 provider。
...
- Atomic steps：
  ...
  2. 暴露 `get_market_pack/get_fundamental_pack/get_news_pack/get_social_pack`。
  ...
  5. 迁移期历史工具名只做 OpenBB gateway wrapper。
...
- Goal：让 frontline worker 只看到领域 pack 工具或迁移期 alias，downstream worker 不看到 OpenBB 数据工具。
...
- Atomic steps：
  1. 迁移期 alias 内部只调用 OpenBB gateway。
...
任务编号：D1 / T5
任务目标：在已存在的 OpenBB submodule/runtime 上实现 extension/MCP pack endpoint，只暴露 get_market_pack/get_fundamental_pack/get_news_pack/get_social_pack，不退回本地 Python data_gateway 直连 provider。
...
5. 迁移期历史工具名只做 OpenBB gateway wrapper，不 import 旧 provider executor。
...
任务编号：D2 / T7
任务目标：让 frontline worker 只看到领域 pack 工具或迁移期 alias，downstream worker 不看到 OpenBB 数据工具，并用 openclaw_llm_provider_payload 证明 LLM 可见 tool schema 边界。
...
- worker visible tool schema 只含领域 pack 工具或迁移期 alias。
...
| CRYPTO | `BTC` | 验证 BB/CoinGlass/Binance/CoinGecko/DefiLlama/news/social source_role |
