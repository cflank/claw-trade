# 数据源修改实施方案（历史口径，已 superseded）

> **SUPERSEDED / 历史背景**
>
> 本文是旧 已删除数据网关 迁移实施方案，当前不再作为实施合同、验收合同或目标运行时口径。
>
> 当前数据层主合同是 `docs/数据层详细设计.md` 和 `docs/数据层实施任务清单.md`：数据层核心为 `src/claw_trade/data_gateway` 的 `DataRequest -> DataResult`；Provider 能力来自 `ProviderPlugin.capabilities()`；Mongo 目标 collection 固定为 8 个；已删除数据网关 本体已删除/不再作为目标运行时依赖；旧 `removed_data_gateway_*` 名称不得作为新目标表名、模块名或证据链名。
>
> 本文中关于 `third_party/removed_data_gateway`、已删除数据网关 submodule、已删除数据网关 runtime/extension/MCP、已删除数据网关 final acceptance 和 `removed_data_gateway_*` evidence 的要求只作为历史背景或 forbidden legacy path 参考，不得用于新实现验收。

## 1. Verdict

可以实施，但不能立即进入编码。当前方案的结论是：**已删除数据网关 必须作为唯一数据源入口、唯一 provider 接口层、唯一数据 MCP，并以 `third_party/removed_data_gateway` submodule/vendor runtime core 进入 claw-trade**。实施前必须先关闭 已删除数据网关 基础设施门（仓库/版本/许可/安全/pack endpoint 承载）；任一门未关闭，后续任务只能做只读盘点和文档校验，不能修改运行代码。

实施前的人类决策门：

- 已删除数据网关 submodule URL：必须给出真实仓库 URL，不能用占位、fork 猜测或浮动分支。
- fixed commit/tag：必须锁定可复现 commit/tag，并写入版本证据。
- license / commercial-use approval：必须明确 已删除数据网关 及每个 provider 的许可、费用、商业使用边界和 raw export policy。
- project extension placement：必须确认项目自有 provider 放在 已删除数据网关 extension 的哪一层，且不把 claw-trade workflow、worker prompt、report exporter 业务逻辑写入 `third_party/removed_data_gateway`。
...
- 已删除数据网关 当前版本是否能承载 extension/MCP pack endpoint：必须至少承载 `claw_get_market_pack/claw_get_fundamental_pack/claw_get_news_pack/claw_get_social_pack`，且在 CN_A 还要承载 `claw_get_policy_pack/claw_get_hot_money_pack/claw_get_lockup_pack`；若不能承载，必须停止并重新设计，不得绕过 已删除数据网关/data_gateway。

A股逐源实现与验收边界：

- 文档列出的 A股源按 已删除数据网关/data_gateway adapter 先进入工程实现，不再把“是否已验证/是否已跑样本”当作人类拍板门。
- 样本、raw、license、attempt/readiness/root-cause 属于实现验收证据；用于证明质量，不用于决定“是否采用”。

## 2. Scope

本实施方案覆盖：

- 引入 `third_party/removed_data_gateway` submodule/vendor runtime core。
- 建立 已删除数据网关 runtime / extension 所有权边界，确保外部行情、基本面、新闻、舆情 provider 都经 已删除数据网关 runtime 或 已删除数据网关 加载的项目 extension。
- 统一 已删除数据网关 native provider 与项目自有 provider 的 `ProviderAdapter` / `ProviderCapability` 合同。
...
- 迁移七域 7-pack：MarketPack、FundamentalPack、NewsPack、SocialPack、PolicyPack、HotMoneyPack、LockupPack（CN_A）。
...
- 不保留旧 MCP / 旧 provider 作为 已删除数据网关 失败后的 silent fallback。
...
- `memory/2026-05-16.md`：CRYPTO/HK live 验证、provider cache、natural-language pack、HK 缺图根因、已删除数据网关 设计演进背景。
- `memory/2026-05-17.md`：已删除数据网关 详细设计的多轮修订、OpenViking 增强边界、当前阻断门、HK `stock_hk_daily` 缺图教训。
- `docs/数据源removed_data_gateway引入方案.md`：本实施方案的主合同。若本方案与该设计文档冲突，以该设计文档为准；若设计文档内部前后冲突，停止实施并先修正文档。

## 4. Human Decision Gates

以下事项必须由人类确认后才能开始 Wave 1 编码：

- 已删除数据网关 submodule URL：真实仓库 URL、远端可访问、不是临时镜像。
- fixed commit/tag：固定 commit SHA 或 release tag；禁止 floating branch。
- license / commercial-use approval：已删除数据网关 许可、已删除数据网关 native provider 许可、项目自有 provider 许可、raw export policy、商业使用批准记录。
- project extension placement：2026-05-17 人类已批准 `src/claw_trade/data_gateway/providers/**` 作为项目 已删除数据网关 extension/shim 放置层，由 已删除数据网关 pack endpoint wrapper/`已删除数据网关RuntimeWrapper` 加载；不得把 claw-trade workflow、worker prompt、PM 结论、report exporter 写进 已删除数据网关。
...
- 已删除数据网关 当前版本是否能承载 extension/MCP pack endpoint：必须证明可暴露领域 pack endpoint；当前批准形态允许项目本地 已删除数据网关 shim 被 pack endpoint wrapper 加载，但不允许 workflow/controller 直接调用 provider。

说明：本节只定义 已删除数据网关 基础设施门，不把 A股源逐个样本验证当作“是否采用”的人类决策。A股源采用范围已在设计文档固定，后续按 adapter 实施并用验收证据证明质量。

每个门的交付证据：

- `docs/evidence/removed_data_gateway-submodule-version.md`：URL、commit/tag、license 摘要、审批人、审批日期、升级和回滚办法。
- `docs/evidence/removed_data_gateway-declarative-provider-security.md`：allowed declarative provider domain policy、协议限制、redirect 复验规则、禁止 localhost/private IP/metadata service 的 SSRF policy、审批人、审批日期、残余风险。
- `git submodule status third_party/removed_data_gateway` 输出。
- 已删除数据网关 extension/MCP capability 证明，至少包含 runtime marker、extension version、pack endpoint 或等价接口截图/日志。
...
- no silent fallback：已删除数据网关 失败后不得在同一 run 静默切回旧 MCP / 旧 provider。
...
- `official_original`（公告原文）coverage_group 失败必须写失败和 root cause，该组不得标 `ready`；但 workflow 不因该组失败被阻断，worker 要继续产出带缺口说明的 L1。
...
- `removed_data_gateway_provider_http_evidence`：只指 已删除数据网关/adapter 调外部数据 provider 时的 HTTP 请求、响应状态、headers 摘要、source URL、provider request id、latency、错误码等运行证据。
- `removed_data_gateway_provider_raw_evidence`：只指外部数据 provider 返回的 raw payload/hash/ref 与 raw export policy 证据。
- `removed_data_gateway_provider_http_evidence` / `removed_data_gateway_provider_raw_evidence` 不能被当成 `openclaw_llm_provider_payload`；HTTP/raw 证据只能证明外部 provider 调用与数据真实性，不能证明 LLM 可见 prompt/tool schema。
- live/fresh 验收必须同时收集两类证据：OpenClaw LLM provider payload 证明模型可见边界，已删除数据网关 HTTP/raw provider evidence 证明数据源调用与 raw/normalized/cache refs。

## 6. Dependency Graph

```mermaid
flowchart TB
  W0["Wave 0: 人类决策与 已删除数据网关 submodule"]
  T0["T0 冻结门与审批证据"]
  T1["T1 引入 third_party/removed_data_gateway"]

  W1["Wave 1: 数据合同 / Mongo store / provider catalog / 已删除数据网关 runtime shim"]
  T2["T2 数据模型与状态合同"]
  T3["T3 Mongo evidence store + persistent single-flight"]
  T4["T4 Provider catalog + declarative admission"]
  T5["T5 已删除数据网关 runtime/extension/MCP wrapper"]
...
- Wave 1 部分并行：T2、T3、T4 可由不同 subagent 并行；T5 只能先做 已删除数据网关 runtime discovery / extension shim 准备，pack endpoint wrapper 接入必须等 T2 的 `ProviderAdapter` / `GatewaySettings` 合同可导入，不能自行定义第二套 adapter 协议。
...
- Goal：关闭 已删除数据网关 URL、commit/tag、license、extension placement、allowed declarative provider domain policy / SSRF policy、MCP pack endpoint 六个开工门。
- Files to read：`AGENTS.md`、`docs/数据源removed_data_gateway引入方案.md`、`.gitmodules`、`pyproject.toml`。
- Files allowed to edit：`docs/evidence/removed_data_gateway-submodule-version.md`、`docs/evidence/removed_data_gateway-declarative-provider-security.md`、`memory/YYYY-MM-DD.md`。
- Files forbidden to edit：`src/**`、`agents/**`、`openclaw_plugins/**`、`third_party/**`、runtime guard 文件。
- Preconditions：人类给出 已删除数据网关 仓库、固定版本、许可审批结论和声明式 provider 域名/SSRF 策略。
- Atomic steps：
  ...
  4. 明确 已删除数据网关 当前版本是否能承载 extension/MCP pack endpoint。
  5. 若任一项缺失，标记为 Human Decision Gate 未关闭。
- Acceptance tests：`test -s docs/evidence/removed_data_gateway-submodule-version.md && test -s docs/evidence/removed_data_gateway-declarative-provider-security.md`；人工复核审批字段完整。
- Evidence to collect：审批文档、许可链接、已删除数据网关 extension/MCP 能力说明、allowed declarative provider domain policy、SSRF policy。
- Stop conditions：任一审批项仍是占位；license 无法接受；已删除数据网关 版本不能承载 pack endpoint。
- Parallelization notes：必须串行；所有后续 subagent 依赖此任务。

### T1：引入 `third_party/removed_data_gateway` submodule

- Goal：把 已删除数据网关 作为受控 submodule/vendor runtime core 纳入仓库。
- Files to read：`docs/evidence/removed_data_gateway-submodule-version.md`、`.gitmodules`、`scripts/start-control-runtime.sh`。
- Files allowed to edit：`.gitmodules`、`third_party/removed_data_gateway` submodule 指针、`scripts/setup-removed_data_gateway-dev.sh`、`.runtime/dev-services/removed_data_gateway/` 配置模板、`docs/evidence/removed_data_gateway-submodule-version.md`。
- Files forbidden to edit：业务 pack、workflow、worker prompt、report exporter、OpenClaw DAG、OpenViking provider 逻辑。
- Preconditions：T0 完成。
- Atomic steps：
  1. `git submodule add <approved-url> third_party/removed_data_gateway`。
  2. `git -C third_party/removed_data_gateway checkout <approved-commit-or-tag>`。
  3. 增加开发安装脚本，只准备 已删除数据网关 runtime，不启动旧 MCP。
  4. 记录 submodule status 与 已删除数据网关 runtime marker。
- Acceptance tests：`git submodule status third_party/removed_data_gateway`；已删除数据网关 runtime 可启动并输出版本；缺 key 时返回 credential_missing 诊断而不是假成功。
- Evidence to collect：submodule status、已删除数据网关 version、runtime marker、extension discovery 输出。
- Stop conditions：只能使用 floating branch；license 不可接受；已删除数据网关 无法承载 extension/MCP pack endpoint。
...
- Files to read：`docs/数据源removed_data_gateway引入方案.md` 第 5、13.2、13.3 章；现有 `openclaw_plugins/.../frontline_data_pack/models.py`。
...
- Atomic steps：
  ...
  6. 写 adapter 合同测试，证明 已删除数据网关 native、project extension、declarative provider 都必须实现同一 `ProviderAdapter` surface。
...
### T5：已删除数据网关 runtime / MCP / wrapper

- Goal：证明并实现 已删除数据网关 runtime/extension/shim ownership，不退回 workflow/controller 直连 provider。
- Files to read：设计文档 13.1.1、13.12、13.12.1；已删除数据网关 extension 文档；现有 `openclaw_plugins/.../openclaw.plugin.json`。
- Files allowed to edit：已删除数据网关 extension shim、`src/claw_trade/data_gateway/mcp/**`、pack tool wrapper、已删除数据网关 setup/runtime 配置、相关 tests。
- Files forbidden to edit：12-worker DAG、workflow 调度顺序、PM/exporter 投资结论、OpenViking provider 行为。
- Preconditions：T1 完成；T2 的 `ProviderAdapter` / `GatewaySettings` 合同可导入；已删除数据网关 pack endpoint 能力确认。
- Atomic steps：
  1. 建立已批准的项目 已删除数据网关 extension/shim package。
  2. 暴露 canonical 资料包 endpoint：`claw_get_market_pack/claw_get_fundamental_pack/claw_get_news_pack/claw_get_social_pack`，并在 CN_A 暴露 `claw_get_policy_pack/claw_get_hot_money_pack/claw_get_lockup_pack`。
  3. 工具返回只输出自然语言 `reader_brief_md`，audit payload 写 evidence。
  4. 已删除数据网关 MCP/pack endpoint 只注册领域 pack endpoint，不注册 atomic provider/admin/discovery/prompt/debug/cache endpoint；OpenClaw worker visible schema 由 T7/D2 验收。
  5. 历史工具名只用于替换盘点和 import-block 验收；worker-visible schema 只暴露 canonical `claw_get_*_pack`。
- Acceptance tests：`uv run pytest tests/unit/data_gateway/test_removed_data_gateway_runtime_wrapper.py tests/integration/data_gateway/test_mcp_pack_endpoints.py`。
- Evidence to collect：已删除数据网关 runtime marker、extension version、pack endpoint audit、wrapper call audit。
- Stop conditions：已删除数据网关 无 pack endpoint；wrapper 需要 import 旧 provider executor；workflow/controller 需要直接调用 provider；worker 必须看到 已删除数据网关 atomic/admin/discovery tool 才能工作。
...
- Goal：让 frontline worker 只看到 canonical `claw_get_*_pack` 领域 pack 工具，downstream worker 不看到 已删除数据网关 数据工具。
...
- Atomic steps：
  1. 禁止历史 pack alias 在 已删除数据网关 flag 开启后进入 frontline tool schema。
  2. 禁止 US atomics 在 已删除数据网关 flag 开启后进入 frontline tool schema。
  3. downstream workers visible tools 为空。
  4. `openclaw_llm_provider_payload` scan 禁止 已删除数据网关 admin/discovery/raw/debug/cache tools。
...
- Evidence to collect：每个 worker visible tool schema、`openclaw_llm_provider_payload` scan 输出、已删除数据网关 gateway audit marker。
- Stop conditions：worker 必须直接看 atomic provider tool；alias 调用旧 executor；downstream worker 看到 已删除数据网关 data tool。
...
- Atomic steps：
  ...
  5. 已删除数据网关 flag 打开时 import-block 旧 market executor。
...
- Goal：实现 CN_A/HK/US/CRYPTO 基本面资料包，保留估值、财务、官方原文、币种、报告期、冲突；并与 CN_A 七域矩阵的公告/新闻/资金/筹码/政策作用域保持边界一致。
...
- Preconditions：T2/T3/T5/T6/T7 完成最小接口。
- Atomic steps：
  1. 建立 CN_A 七域系统默认源矩阵（公告/新闻/行情/财务/资金/筹码/政策）中的 fundamental 作用域 capability；Tushare 仅作为用户配置并验证启用后的同组可选源，不作为默认交付前提。
...
- Files forbidden to edit：worker tool schema、OpenClaw prompt、provider direct code upload、已删除数据网关 runtime bypass。
- Preconditions：T4 完成。
- Atomic steps：
...
- Goal：找出旧 MCP / 旧 provider / 旧 cache / US atomics / direct provider imports，并建立 已删除数据网关 flag 下的 import-block 负面证明。
...
- Atomic steps：
  ...
  2. 建 已删除数据网关 flag 下 import-block 测试，旧模块被 monkeypatch/import-block 成抛错时，已迁移 pack 必须仍经 已删除数据网关 runtime 成功或显式失败。
...
- Stop conditions：旧路径仍参与 已删除数据网关 flag 下成功；compare mode 结果进入 worker 正文；silent fallback 存在。
...
- Atomic steps：
  ...
  2. 运行时默认且只能使用 已删除数据网关。
  3. 当前主线不保留旧 provider 可运行回滚模式，metadata 保持 `data_gateway=removed_data_gateway`；需要回旧版只能人工 Git/VCS 回退。
  4. 同一 `/report` run 禁止 已删除数据网关 失败后切旧路径装成功。
- Acceptance tests：对应 pack import-block tests、tool schema tests、live/fresh smoke。
- Evidence to collect：删除/隔离 diff、旧路径运行边界 rg 结果、Git/VCS 回滚文档、`data_gateway=removed_data_gateway` 元数据样例。
- Stop conditions：删除需要一次性大爆炸；旧路径仍作为同 run fallback；回滚吞掉 已删除数据网关 失败原因。
...
- Atomic steps：
  ...
  2. 验证 downstream workers 无 已删除数据网关/OpenViking/Mongo raw tools。
...
- Evidence to collect：`openclaw_llm_provider_payload`、tool schema、exporter read trace、final report evidence chain；如引用外部数据源证据，必须另列 `removed_data_gateway_provider_http_evidence` / `removed_data_gateway_provider_raw_evidence`。
...
- Atomic steps：
  ...
  5. 分开收集 `openclaw_llm_provider_payload`、tool schema、tool calls、`removed_data_gateway_provider_http_evidence`、`removed_data_gateway_provider_raw_evidence`、Mongo refs、run plan、OpenViking lineage、ovpack、runtime health、chart readiness、final report。
- Acceptance tests：真实 live/fresh 命令和四市场 evidence review。
- Evidence to collect：每个 run 的 preflight、run_id、`openclaw_llm_provider_payload`、`removed_data_gateway_provider_http_evidence`、`removed_data_gateway_provider_raw_evidence`、Mongo documents、OpenViking tree/grep/glob/relations、ovpack receipt、final_report。
...
```text
你是 claw-trade 已删除数据网关 数据源迁移的 Subagent A。你不是唯一 agent，不得 revert 他人改动。只处理 assigned scope：T2 数据模型与状态合同。
...
读取文件：AGENTS.md；docs/数据源removed_data_gateway引入方案.md；docs/数据源修改实施方案.md；openclaw_plugins/claw-trade-frontline-tools/python/frontline_data_pack/models.py。
...
输入依赖：T1 完成；已删除数据网关 submodule/runtime 事实已记录；本任务不需要真实 provider 调用。
...
3. 在 providers/base.py 定义唯一 ProviderAdapter protocol；已删除数据网关 native、project extension、declarative provider 都只能实现这套协议。
...
```text
你是 claw-trade 已删除数据网关 数据源迁移的 Subagent B。你不是唯一 agent，不得 revert 他人改动。只处理 assigned scope：T3 Mongo evidence store 与 persistent single-flight。

任务编号：B / T3
任务目标：实现 removed_data_gateway_provider_manifests、removed_data_gateway_provider_validation_receipts、removed_data_gateway_provider_attempts、removed_data_gateway_raw_payloads、removed_data_gateway_cache_entries、removed_data_gateway_normalized、removed_data_gateway_rate_limits、removed_data_gateway_run_provider_plans、removed_data_gateway_single_flight_calls 的 store 合同。
读取文件：AGENTS.md；docs/数据源removed_data_gateway引入方案.md；docs/数据源修改实施方案.md；openclaw_plugins/.../mongo_store.py；crypto_provider_cache.py；normalized_store.py。
...
```text
你是 claw-trade 已删除数据网关 数据源迁移的 Subagent C。你不是唯一 agent，不得 revert 他人改动。只处理 assigned scope：T4 与 T12 的 provider catalog / declarative admission 后端。
...
读取文件：AGENTS.md；docs/数据源removed_data_gateway引入方案.md；docs/数据源修改实施方案.md；现有 provider_specs.py；相关 UI/provider settings。
...
禁止修改文件范围：pack builders、OpenClaw stage policy、worker prompt、已删除数据网关 runtime shim、旧路径删除。
...
### Subagent D0：已删除数据网关 submodule 引入（Wave 0 串行）

```text
你是 claw-trade 已删除数据网关 数据源迁移的 Subagent D0。你不是唯一 agent，不得 revert 他人改动。只处理 assigned scope：T1 已删除数据网关 submodule 引入。

任务编号：D0 / T1
任务目标：把 已删除数据网关 作为 third_party/removed_data_gateway submodule/vendor runtime core 接入，并记录可复现版本事实。
读取文件：AGENTS.md；docs/数据源removed_data_gateway引入方案.md；docs/数据源修改实施方案.md；docs/evidence/removed_data_gateway-submodule-version.md；.gitmodules；scripts/start-control-runtime.sh。
允许修改文件范围：.gitmodules；third_party/removed_data_gateway submodule pointer；scripts/setup-removed_data_gateway-dev.sh；.runtime/dev-services/removed_data_gateway/ 配置模板；docs/evidence/removed_data_gateway-submodule-version.md；memory/YYYY-MM-DD.md。
...
输出交付物：已删除数据网关 submodule、submodule status、已删除数据网关 runtime marker、版本/许可/extension 承载能力证据。
原子步骤：
1. 根据已批准 URL/commit/tag 添加 third_party/removed_data_gateway submodule。
2. 锁定 submodule 指针并写入 status。
3. 增加开发安装脚本，只准备 已删除数据网关 runtime，不启动旧 MCP。
4. 记录 已删除数据网关 runtime marker 与 extension/MCP pack endpoint 承载能力。
验收命令：git submodule status third_party/removed_data_gateway
验收证据：submodule status、已删除数据网关 version/runtime marker、许可审批引用。
停止条件：T0 任一门未关闭；只能使用 floating branch；license 不可接受；已删除数据网关 无法承载 extension/MCP pack endpoint。
与其他 subagent 的冲突边界：D0 只拥有 T1；D1 才做 已删除数据网关 runtime/MCP wrapper；D2 才做 OpenClaw tool schema。

完成报告必须包含 changed files、commands、exit code、key output、expected outcome、deviation status、real implementation status、mock/stub/fake/fallback status、remaining risk，并追加 memory/YYYY-MM-DD.md。
```

### Subagent D1：已删除数据网关 runtime / MCP / pack endpoint

```text
你是 claw-trade 已删除数据网关 数据源迁移的 Subagent D1。你不是唯一 agent，不得 revert 他人改动。只处理 assigned scope：T5 已删除数据网关 runtime / MCP / wrapper。

任务编号：D1 / T5
任务目标：在已存在的 已删除数据网关 submodule/runtime 上实现 extension/MCP pack endpoint；默认暴露 `claw_get_market_pack/claw_get_fundamental_pack/claw_get_news_pack/claw_get_social_pack`，并在 CN_A 暴露 `claw_get_policy_pack/claw_get_hot_money_pack/claw_get_lockup_pack`；不得绕过 已删除数据网关/data_gateway。
读取文件：AGENTS.md；docs/数据源removed_data_gateway引入方案.md；docs/数据源修改实施方案.md；docs/evidence/removed_data_gateway-submodule-version.md；src/claw_trade/data_gateway/providers/base.py。
允许修改文件范围：已删除数据网关 extension shim；src/claw_trade/data_gateway/mcp/**；pack tool wrapper；已删除数据网关 setup/runtime 配置；tests/unit/data_gateway/test_removed_data_gateway_runtime_wrapper.py；tests/integration/data_gateway/test_mcp_pack_endpoints.py。
禁止修改文件范围：.gitmodules；third_party/removed_data_gateway submodule pointer；12-worker DAG ownership；worker prompts；PM/exporter 投资结论；provider adapter business logic；OpenViking provider 行为。
输入依赖：D0/T1 完成；Subagent A 的 ProviderAdapter/GatewaySettings 合同可导入。
输出交付物：已删除数据网关 runtime marker、extension version、pack endpoints、wrapper call audit。
原子步骤：
1. 建 已删除数据网关 extension package 或 tree shim。
2. 暴露 canonical endpoint：`claw_get_market_pack/claw_get_fundamental_pack/claw_get_news_pack/claw_get_social_pack`，并在 CN_A 增加 `claw_get_policy_pack/claw_get_hot_money_pack/claw_get_lockup_pack`。
3. 工具返回只输出自然语言 reader_brief_md，audit payload 写 evidence。
4. 屏蔽 已删除数据网关 atomic provider/admin/discovery/prompt tools 进入 worker schema 的输入材料；D2 再负责 OpenClaw visible tool schema 验收。
...
验收命令：uv run pytest tests/unit/data_gateway/test_removed_data_gateway_runtime_wrapper.py tests/integration/data_gateway/test_mcp_pack_endpoints.py
验收证据：已删除数据网关 runtime marker、extension version、pack endpoint audit、无旧 provider executor import 证据。
停止条件：已删除数据网关 无法承载 extension/MCP pack endpoint；只能本地 Python 直连 provider；wrapper 需要旧 provider_executor fallback；需要新增第二套 ProviderAdapter 协议。
...
```text
你是 claw-trade 已删除数据网关 数据源迁移的 Subagent D2。你不是唯一 agent，不得 revert 他人改动。只处理 assigned scope：T7 OpenClaw tool schema 与 pack wrapper 接入。

任务编号：D2 / T7
任务目标：让 frontline worker 只看到 canonical `claw_get_*_pack` 领域 pack 工具，downstream worker 不看到 已删除数据网关 数据工具，并用 openclaw_llm_provider_payload 证明 LLM 可见 tool schema 边界。
读取文件：AGENTS.md；docs/数据源removed_data_gateway引入方案.md；docs/数据源修改实施方案.md；openclaw_plugins/claw-trade-frontline-tools/openclaw.plugin.json；src/claw_trade/config/tool_names.py；`openclaw_llm_provider_payload` 证据样例。
...
禁止修改文件范围：.gitmodules；third_party/removed_data_gateway submodule pointer；已删除数据网关 provider adapter business logic；12-worker DAG；worker prompts；report exporter；runtime guards。
输入依赖：D1/T5 pack endpoint 可调用；Subagent A 模型合同可导入。
输出交付物：worker visible tool schema 白名单、`openclaw_llm_provider_payload` scan、已删除数据网关 gateway audit marker。
原子步骤：
1. 禁止历史 pack alias 进入 已删除数据网关 flag 下的 frontline tool schema。
2. 禁止 US atomics 和 已删除数据网关 atomic/admin/discovery/raw/debug/cache tools 进入 已删除数据网关 flag 下的 frontline tool schema。
3. downstream workers visible tools 为空。
4. `openclaw_llm_provider_payload` scan 只判断 LLM 请求 messages/tool schema，不把 `removed_data_gateway_provider_http_evidence` / `removed_data_gateway_provider_raw_evidence` 当 LLM payload。
验收命令：uv run pytest tests/unit/data_gateway/test_tool_schema.py tests/integration/data_gateway/test_mcp_visible_tools.py
验收证据：每个 worker visible tool schema、openclaw_llm_provider_payload scan 输出、已删除数据网关 gateway audit marker。
停止条件：worker 必须直接看 atomic provider tool；alias 调用旧 executor；downstream worker 看到 已删除数据网关 data tool；把 removed_data_gateway_provider_http/raw evidence 误当成 OpenClaw LLM provider payload。
与其他 subagent 的冲突边界：D2 拥有 OpenClaw tool schema 和 `openclaw_llm_provider_payload` scan；D1 拥有 已删除数据网关 runtime/MCP；J 只删除已迁移旧路径。
...
```text
你是 claw-trade 已删除数据网关 数据源迁移的 Subagent E。你不是唯一 agent，不得 revert 他人改动。只处理 assigned scope：T6 RunProviderPlan workflow 接入。
...
读取文件：AGENTS.md；docs/数据源removed_data_gateway引入方案.md；docs/数据源修改实施方案.md；src/claw_trade/workflow/**；scripts/run_claw_trade_fresh_report.py。
...
```text
你是 claw-trade 已删除数据网关 数据源迁移的 Subagent F。你不是唯一 agent，不得 revert 他人改动。只处理 assigned scope：T8 MarketPack。
...
读取文件：AGENTS.md；docs/数据源removed_data_gateway引入方案.md；docs/数据源修改实施方案.md；memory/2026-05-17.md；现有 market_data_pack.py、crypto_market_data_pack.py、normalizer_market.py。
...
停止条件：stock_hk_daily 前导零丢失；图表缺失靠 exporter 绕过；已删除数据网关 flag 下调用旧 market executor。
...
```text
你是 claw-trade 已删除数据网关 数据源迁移的 Subagent G。你不是唯一 agent，不得 revert 他人改动。只处理 assigned scope：T9 FundamentalPack。
...
读取文件：AGENTS.md；docs/数据源removed_data_gateway引入方案.md；docs/数据源修改实施方案.md；现有 fundamentals_data_pack.py、crypto_fundamental_data_pack.py。
...
原子步骤：
1. 配置 CN_A 七域系统默认源矩阵里的 fundamental 作用域（财务/估值）能力；Tushare 仅在用户配置、验证、启用后作为同 `market/domain/source_role/coverage_group` 可选优先源；同时完成 HK official/financial、US FMP/SEC、CRYPTO CoinGecko/DefiLlama。
...
```text
你是 claw-trade 已删除数据网关 数据源迁移的 Subagent H。你不是唯一 agent，不得 revert 他人改动。只处理 assigned scope：T10 NewsPack / SocialPack。

任务编号：H / T10
任务目标：实现新闻事实源、官方原文、global/macro news、search discovery、社交原始样本、聚合情绪、Alternative.me、Polymarket 的 source_role 边界。
读取文件：AGENTS.md；docs/数据源removed_data_gateway引入方案.md；docs/数据源修改实施方案.md；现有 news_data_pack.py、social_sentiment_pack.py、crypto_news_data_pack.py、crypto_social_sentiment_pack.py。
...
```text
你是 claw-trade 已删除数据网关 数据源迁移的 Subagent I。你不是唯一 agent，不得 revert 他人改动。只处理 assigned scope：T11 OpenViking material/context/runtime health。
...
读取文件：AGENTS.md；docs/数据源removed_data_gateway引入方案.md；docs/数据源修改实施方案.md；src/claw_trade/artifacts/openviking_client.py；src/claw_trade/runtime/openviking_report_server.py。
...
```text
你是 claw-trade 已删除数据网关 数据源迁移的 Subagent J。你不是唯一 agent，不得 revert 他人改动。只处理 assigned scope：T13/T14 旧路径盘点、import-block 负面证明和按 pack 删除。

任务编号：J / T13/T14
任务目标：盘点旧 MCP / 旧 provider / 旧 cache / direct provider imports，建立 已删除数据网关 flag 下 import-block 负面证明，并在 pack live/fresh 通过后按 pack 删除旧路径。
读取文件：AGENTS.md；docs/数据源removed_data_gateway引入方案.md；docs/数据源修改实施方案.md；openclaw_plugins/**；agents/*/skills/**/scripts/**；历史 `src/claw_trade/providers/**` 删除证据；src/claw_trade/config/tool_names.py。
...
2. 已删除数据网关 flag 下 monkeypatch/import-block 旧模块，已迁移 pack 仍必须 已删除数据网关 成功或显式失败。
...
验收证据：rg hit list、import-block 输出、删除/隔离 diff、Git/VCS 回滚文档和 `data_gateway=removed_data_gateway` metadata。
...
```text
你是 claw-trade 已删除数据网关 数据源迁移的 Subagent K。你不是唯一 agent，不得 revert 他人改动。只处理 assigned scope：T16 四市场 live/fresh 验收。注意：live-run 子代理不得自证 runtime ready；manager 必须先打印 fixed runtime preflight 表。

任务编号：K / T16
任务目标：按 CN_A 600519、HK 00700.HK + 非腾讯样本、US AAPL/MSFT、CRYPTO BTC 做真实 live/fresh collect-first 验收。
读取文件：AGENTS.md；docs/数据源removed_data_gateway引入方案.md；docs/数据源修改实施方案.md；memory/2026-05-16.md；memory/2026-05-17.md；scripts/start-control-runtime.sh。
...
输出交付物：四市场 run evidence、preflight、`openclaw_llm_provider_payload`、`removed_data_gateway_provider_http_evidence`、`removed_data_gateway_provider_raw_evidence`、tool schema、tool calls、Mongo docs、RunProviderPlan、OpenViking lineage、ovpack、runtime health、chart readiness、final report。
原子步骤：
...
验收证据：run_id、preflight 表、`openclaw_llm_provider_payload`、`removed_data_gateway_provider_http_evidence`、`removed_data_gateway_provider_raw_evidence`、tool schema、Mongo refs、OpenViking tree/grep/glob/relations、ovpack receipt、final_report.md。
...
3. 再 pack integration：D1/D2/E 建 已删除数据网关 runtime wrapper、OpenClaw tool schema 与 no-prefetch run plan 后，F/G/H 才迁移 Market/Fundamental/News/Social。
4. 再 OpenClaw payload：T7/T15 验证 `openclaw_llm_provider_payload` 和 visible tool schema，确认 worker 只见领域 pack，自然语言资料包不含 JSON/debug/cache/Mongo raw/OpenViking protocol；已删除数据网关 HTTP/raw evidence 另行用于数据源真实性验证。
...
- 已删除数据网关 runtime/MCP 冲突回到 Subagent D1；OpenClaw tool schema / openclaw_llm_provider_payload scan 冲突回到 Subagent D2。
...
- 已删除数据网关 flag 开启后旧 MCP / 旧 provider 不能 silent fallback。
...
| 市场 | 样本 | 目标 |
|---|---|---|
| CN_A | `600519` | A股七域全链路（market/fundamental/news/social/policy/hot_money/lockup）与 7-pack endpoint |
| HK | `00700.HK` + 非腾讯样本 | 验证 HK `stock_hk_daily`、图表 readiness、公告/新闻 |
| US | `AAPL` 或 `MSFT` | 验证 已删除数据网关 native FMP/Polygon/SEC/FRED 边界 |
| CRYPTO | `BTC` | 验证 已删除数据网关 CRYPTO provider attempts（CoinGlass/Binance/CoinGecko/DefiLlama/news/social source_role）与 CryptoLens analysis evidence 边界 |
...
- `removed_data_gateway_provider_http_evidence`：已删除数据网关/adapter 调外部数据 provider 的 HTTP 请求/响应证据。
- `removed_data_gateway_provider_raw_evidence`：外部 provider raw payload/hash/ref 证据。
...
- 已删除数据网关 不能作为唯一数据入口，需要保留长期旧 MCP 并行入口。
- 已删除数据网关 submodule URL、commit/tag、license 审批未确认或仍是占位。
- 已删除数据网关 license/AGPL/商业使用风险无法接受。
- 已删除数据网关 当前版本无法承载 extension/MCP pack endpoint，且继续实施只能绕过 已删除数据网关/data_gateway 时，必须停止并重新设计。
- 已迁移 pack 在 已删除数据网关 flag 打开时仍需要调用旧 provider executor 或旧 MCP 兜底。
...
- worker 必须直接看到 已删除数据网关 atomic provider tools、Mongo raw、OpenViking URI、JSON/debug envelope 才能工作。
...
- `已删除数据网关_DATA_GATEWAY_ENABLED`
- `已删除数据网关_DATA_PACK_<MARKET>_<DOMAIN>_ENABLED`
- `已删除数据网关_DATA_COMPARE_MODE`

删除策略：

- 按 pack 删除旧路径，不做一次性大爆炸。
- 每个 pack 先完成 已删除数据网关 wrapper、compare evidence、`openclaw_llm_provider_payload`、`removed_data_gateway_provider_http_evidence` / `removed_data_gateway_provider_raw_evidence`、Mongo evidence、OpenViking lineage、live/fresh 证明。
- 删除前必须有 import-block 测试：旧 executor/direct provider 被 block 后，已删除数据网关 pack 仍成功或显式失败。
- 删除后 `rg` 旧路径关键字必须只剩迁移文档或未迁移 pack。

回滚策略：

- 回滚是短期人工运维动作，不是长期双入口。
- 当前主线不提供旧 provider 回滚 run；metadata 保持 `data_gateway=removed_data_gateway`。如需回旧版，只能通过 Git/VCS 运维流程并另行记录回退提交和原因。
- 回滚不得吞掉 已删除数据网关 失败原因；失败样本必须留作修复输入。
- 同一 `/report` run 内不允许 已删除数据网关 失败后 silent fallback 到旧 MCP/旧 provider。

## 13. Final Acceptance Criteria

完成定义：

- 已删除数据网关 是唯一数据入口、唯一 provider 接口层、唯一数据 MCP。
- `third_party/removed_data_gateway` 固定 commit/tag、license、extension placement、runtime marker 证据完整。
- 旧 provider 直连和旧 MCP 按 pack 删除；已删除数据网关 flag 下 import-block 证明旧路径不可用。
- 四市场 fresh/live 证据完整：CN_A、HK、US、CRYPTO 都有 `openclaw_llm_provider_payload`、tool schema、tool calls、`removed_data_gateway_provider_http_evidence`、`removed_data_gateway_provider_raw_evidence`、Mongo evidence、OpenViking lineage、ovpack、runtime health、chart readiness、final report。
- worker 只看自然语言资料包或 approved L1；不看 JSON/debug envelope/cache object/Mongo raw/OpenViking protocol。
- downstream worker 不看 已删除数据网关 数据工具；OpenClaw 只承载单 worker turn。
...
- Mongo / OpenViking / `openclaw_llm_provider_payload` / 已删除数据网关 HTTP/raw evidence / final report evidence chain 闭合：final report -> PM L1 -> worker L1 -> L2 evidence -> pack audit -> attempt -> raw/normalized/cache refs。

## 14. Residual Risks

- 已删除数据网关 upstream extension/MCP 能力可能与本设计需要的 pack endpoint 不完全匹配；若不匹配，不能用本地 Python 直连绕过，只能停下重设 extension 形态。
- 已删除数据网关 license 或 provider license 可能限制 raw payload 保存/导出，导致 ovpack 只能 `metadata_verified` 而不能 `complete_portable`。
- HK `stock_hk_daily` 虽能解决 00700.HK 缺图样本，但其它港股可能仍有字段漂移、复权差异、限流或交易日窗口问题。
- US 已删除数据网关 native provider 的字段命名、单位、权限和 rate limit 可能与现有 pack 期望不同，需要 schema drift 测试和 live proof。
