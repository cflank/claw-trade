...
> **SUPERSEDED / 历史背景**
>
> 本文是旧 OpenBB 迁移实施方案，当前不再作为实施合同、验收合同或目标运行时口径。
>
...
>
> 本文中关于 `third_party/openbb`、OpenBB submodule、OpenBB runtime/extension/MCP、OpenBB final acceptance 和 `openbb_*` evidence 的要求只作为历史背景或 forbidden legacy path 参考，不得用于新实现验收。

## 1. Verdict

可以实施，但不能立即进入编码。当前方案的结论是：**OpenBB 必须作为唯一数据源入口、唯一 provider 接口层、唯一数据 MCP，并以 `third_party/openbb` submodule/vendor runtime core 进入 claw-trade**。实施前必须先关闭 OpenBB 基础设施门（仓库/版本/许可/安全/pack endpoint 承载）；任一门未关闭，后续任务只能做只读盘点和文档校验，不能修改运行代码。
...
- OpenBB 当前版本是否能承载 extension/MCP pack endpoint：必须至少承载 `claw_get_market_pack/claw_get_fundamental_pack/claw_get_news_pack/claw_get_social_pack`，且在 CN_A 还要承载 `claw_get_policy_pack/claw_get_hot_money_pack/claw_get_lockup_pack`；若不能承载，必须停止并重新设计，不得绕过 OpenBB/data_gateway。

A股逐源实现与验收边界：

- 文档列出的 A股源按 OpenBB/data_gateway adapter 先进入工程实现，不再把“是否已验证/是否已跑样本”当作人类拍板门。
...
说明：本节只定义 OpenBB 基础设施门，不把 A股源逐个样本验证当作“是否采用”的人类决策。A股源采用范围已在设计文档固定，后续按 adapter 实施并用验收证据证明质量。
...
任务编号：D1 / T5
任务目标：在已存在的 OpenBB submodule/runtime 上实现 extension/MCP pack endpoint；默认暴露 `claw_get_market_pack/claw_get_fundamental_pack/claw_get_news_pack/claw_get_social_pack`，并在 CN_A 暴露 `claw_get_policy_pack/claw_get_hot_money_pack/claw_get_lockup_pack`；不得绕过 OpenBB/data_gateway。
...
- OpenBB 当前版本无法承载 extension/MCP pack endpoint，且继续实施只能绕过 OpenBB/data_gateway 时，必须停止并重新设计。
