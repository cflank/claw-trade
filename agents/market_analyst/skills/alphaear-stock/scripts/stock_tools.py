...
DEFAULT_FUNDAMENTALS_TIMEOUT_SECONDS = float(
    os.getenv("STOCK_FUNDAMENTALS_TIMEOUT_SECONDS", "15")
)
...
class StockTools:
    ...
    def search_ticker(self, query: str, limit: int = 5) -> List[Dict]:
        ...
        cached_matches = self.db.search_stock(search_query, limit)
        if cached_matches:
            return cached_matches

        if search_query.isdigit() and len(search_query) in {5, 6}:
            direct_match = self._lookup_stock_by_code(search_query)
            if direct_match is not None:
                return [direct_match]
            return []

        # 中文注释：仅在模糊名称查询且本地缓存为空时，才补做一次全量股票列表刷新。
        self._check_and_update_stock_list()
        return self.db.search_stock(search_query, limit)

    def _lookup_stock_by_code(self, code: str) -> Optional[Dict[str, str]]:
        cached = self.db.get_stock_by_code(code)
        if cached:
            return cached

        info = self.get_stock_fundamentals(code)
        name = str(info.get("name") or "").strip()
        clean_code = "".join(filter(str.isdigit, str(info.get("code") or code)))
        if not clean_code or not name:
            return None
        self.db.upsert_stock(clean_code, name)
        return {"code": clean_code, "name": name}
    ...
    def get_stock_price(
        ...
        if df_db.empty:
            ...
            db_earliest = pd.to_datetime(df_db['date'].min())
            req_latest = pd.to_datetime(end_date)
            req_earliest = pd.to_datetime(start_date)
            if (req_latest - db_latest).days > 2:
                need_update = True
            if (db_earliest - req_earliest).days > 2:
                need_update = True
        ...
        if need_update:
            ...
            try:
                ...
                def _normalize_price_frame(df_remote_raw: pd.DataFrame) -> pd.DataFrame:
                    normalized = df_remote_raw.copy()
                    if normalized.empty:
                        return normalized
                    ...
                    if "volume" not in normalized.columns:
                        source_volume = normalized["amount"] if "amount" in normalized.columns else 0
                        normalized["volume"] = pd.to_numeric(source_volume, errors="coerce")
                    else:
                    ...
                    normalized["change_pct"] = pd.to_numeric(normalized["change_pct"], errors="coerce").fillna(0)
                    return normalized[["date", "open", "close", "high", "low", "volume", "change_pct"]]

                def _a_share_symbol() -> str:
                    prefix = "sh" if clean_ticker.startswith(("5", "6", "9")) else "sz"
                    return f"{prefix}{clean_ticker}"

                def fetch_data_sina():
                    """备用数据源: 新浪财经历史行情"""
                    logger.info(f"📡 Trying Sina daily for {clean_ticker}...")
                    return _normalize_price_frame(
                        ak.stock_zh_a_daily(
                            symbol=_a_share_symbol(),
                            start_date=s_fmt,
                            end_date=e_fmt,
                            adjust="qfq",
                        )
                ...
                def fetch_data_tencent():
                    """备用数据源: 腾讯证券历史行情"""
                    logger.info(f"📡 Trying Tencent daily for {clean_ticker}...")
                    return _normalize_price_frame(
                ...
                def fetch_additional_cn_sources(primary_error: Exception | None = None):
                    last_source_error = primary_error
                    for label, fetcher in (
                        ("EastMoney direct", fetch_data_eastmoney),
                        ("Sina daily", fetch_data_sina),
                        ("Tencent daily", fetch_data_tencent),
                    ):
                        try:
                            candidate = fetcher()
                        except Exception as exc:
                            last_source_error = exc
                            logger.warning(f"⚠️ {label} also failed for {clean_ticker}: {exc}")
                            continue
                        if candidate is not None and not candidate.empty:
                            logger.info(f"✅ {label}: fetched {len(candidate)} rows for {clean_ticker}.")
                            return candidate
                        logger.warning(f"⚠️ {label} returned empty data for {clean_ticker}")
                ...
                try:
                    ...
                    if not is_us_stock and len(clean_ticker) == 6:
                        df_remote = fetch_additional_cn_sources(e)
                    else:
                        raise e

                if df_remote is not None and df_remote.empty and not is_us_stock and len(clean_ticker) == 6:
                    logger.warning(f"⚠️ akshare returned empty data for {clean_ticker}. Trying additional CN sources...")
                    df_remote = fetch_additional_cn_sources()
        ...
        if is_us_stock:
            ...
            try:
                info = run_with_timeout(
                    f"yfinance fundamentals {clean_ticker}",
                    lambda: yf.Ticker(clean_ticker).info,
                    timeout_seconds=DEFAULT_FUNDAMENTALS_TIMEOUT_SECONDS,
                )
                ...
                logger.error(f"❌ yfinance fundamentals timed out for {clean_ticker}: {e}")
                return {
                    "code": clean_ticker,
                    "error": "fundamentals_timeout",
                    "source": "yfinance",
                    "reason": str(e),
                }
            except Exception as e:
            ...
            try:
                # 使用东财接口获取个股基本信息
                df_info = run_with_timeout(
                    f"akshare fundamentals {clean_ticker}",
                    lambda: ak.stock_individual_info_em(symbol=clean_ticker),
                    timeout_seconds=DEFAULT_FUNDAMENTALS_TIMEOUT_SECONDS,
                )
                ...
                payload.update(self._get_cn_financial_snapshot(clean_ticker))
                return payload
            except TimeoutError as e:
                logger.error(f"❌ akshare fundamentals timed out for {clean_ticker}: {e}")
                return {
                    "code": clean_ticker,
                    "error": "fundamentals_timeout",
                    "source": "akshare",
    ...
    def _get_cn_financial_snapshot(self, ticker: str) -> Dict:
        try:
            df = run_with_timeout(
                f"akshare financial snapshot {ticker}",
                lambda: ak.stock_financial_abstract(symbol=ticker),
                timeout_seconds=DEFAULT_FUNDAMENTALS_TIMEOUT_SECONDS,
            )
        except TimeoutError as exc:
            logger.warning(f"⚠️ akshare financial abstract timed out for {ticker}: {exc}")
            return {
                "financial_snapshot_error": "financial_snapshot_timeout",
                "financial_snapshot_reason": str(exc),
            }
        except Exception as exc:
            logger.warning(f"⚠️ akshare financial abstract failed for {ticker}: {exc}")
            return {}
        if df is None or df.empty:
            return {}

        period_columns = [column for column in df.columns if re.fullmatch(r"\d{8}", str(column))]
        if not period_columns:
            return {}
        latest_period = max(period_columns)
        ...
        snapshot: Dict[str, object] = {"financial_period": latest_period}
        for metric_name, target_key in metric_map.items():
            value = self._extract_financial_metric(df, metric_name, latest_period)
            if value is not None:
                snapshot[target_key] = value
        return snapshot

    def _extract_financial_metric(self, df: pd.DataFrame, metric_name: str, period: str):
        rows = df[df["指标"] == metric_name]
        if rows.empty or period not in rows.columns:
            return None
        raw_value = rows.iloc[0][period]
        if pd.isna(raw_value):
            return None
        return raw_value.item() if hasattr(raw_value, "item") else raw_value


def run_with_timeout(label: str, func, *, timeout_seconds: float):
    """Run a blocking third-party call with a hard frontend timeout."""
    ...
    worker = threading.Thread(target=_target, name=f"alphaear-stock:{label}", daemon=True)
    worker.start()
