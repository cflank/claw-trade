...
def test_metric_claim_without_fact_or_field_source_is_unsupported() -> None:
    ...
    evidence["facts"]["valuation"]["pb"] = None
    evidence["field_sources"].pop("valuation.pb")
    result = evaluate_fundamental_report_claims_v1("公司PB约5倍。", evidence)
...
def test_trend_claim_requires_financial_trend_capability_available() -> None:
    evidence = _base_evidence()
    evidence["evidence_capabilities"]["financial_trend"]["status"] = "blocked"
    result = evaluate_fundamental_report_claims_v1("公司ROE连续三年提升。", evidence)
    assert result.guard.ok is False
    assert len(result.unsupported_claims) == 1
...
def test_narrative_claim_is_not_hard_gated_when_business_segments_evidence_insufficient() -> None:
    evidence = _base_evidence()
    evidence["facts"]["business_segments"] = []
    evidence["field_sources"].pop("business_segments.items")
    result = evaluate_fundamental_report_claims_v1("公司具备护城河，且行业龙头地位稳固。", evidence)
...
def test_metric_gap_statements_are_not_treated_as_unsupported_metric_claims() -> None:
    evidence = _base_evidence()
    evidence["facts"]["financial_indicators"]["gross_margin"] = None
    evidence["facts"]["financial_indicators"]["netprofit_margin"] = None
    evidence["facts"]["financial_indicators"]["debt_to_assets"] = None
    evidence["field_sources"].pop("financial_indicators.gross_margin")
    evidence["field_sources"].pop("financial_indicators.netprofit_margin")
    evidence["field_sources"].pop("financial_indicators.debt_to_assets")
    result = evaluate_fundamental_report_claims_v1(
...
def test_main_evidence_gap_sentence_does_not_trigger_metric_unsupported_claim() -> None:
    ...
    evidence["facts"]["financial_indicators"]["netprofit_margin"] = None
    evidence["facts"]["financial_indicators"]["debt_to_assets"] = None
    evidence["facts"]["financial_indicators"]["roa"] = None
    evidence["field_sources"].pop("financial_indicators.gross_margin")
    evidence["field_sources"].pop("financial_indicators.netprofit_margin")
    evidence["field_sources"].pop("financial_indicators.debt_to_assets")
    evidence["field_sources"].pop("financial_indicators.roa")
    result = evaluate_fundamental_report_claims_v1(
