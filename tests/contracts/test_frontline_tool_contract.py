...
def test_frontline_skill_manifest_only_exports_worker_pack_tool() -> None:
    for worker_id, expected_pack_tool in EXPECTED_WORKER_PACK_EXPORTS.items():
        ...
        assert len(export_entries) == 1, worker_id
        entry = export_entries[0]
        assert entry.get("workers") == [worker_id]
        assert entry.get("tool_exports") == [expected_pack_tool]
...
def test_openviking_write_material_schema_for_non_pm_workers_exposes_only_content() -> None:
    ...
    assert "return isPmDecisionScope(context) ? WRITE_MATERIAL_PM_PARAMETERS : WRITE_MATERIAL_BASE_PARAMETERS;" in source


def test_frontline_plugin_registers_four_pack_tools_with_pack_tool_params_schema() -> None:
