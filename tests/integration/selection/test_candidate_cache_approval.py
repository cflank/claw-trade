...
from claw_trade.selection.candidate_cache import approve_candidate_cache, build_candidate_cache
...
def test_candidate_cache_approval_uses_real_write_readback_hash_manifest_lineage(tmp_path: Path) -> None:
    plan = _plan()
    ...
    approved = approve_candidate_cache(
        plan=plan,
        draft=draft,
    ...
    assert manifest.cache_body_sha256 == ref.content_sha256
    ...
    assert manifest.stage == "approving_candidate_cache"
    assert manifest.target == "candidate_cache"
    ...
    assert manifest_payload["stage"] == "approving_candidate_cache"
    assert manifest_payload["target"] == "candidate_cache"
    assert manifest_payload["readback_status"] == "verified"
    assert manifest_payload["candidate_count"] == 20
    assert manifest_payload["cache_body_sha256"] == ref.content_sha256
