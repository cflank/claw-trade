...
def test_build_candidate_cache_contract_produces_fact_only_model_visible_payload() -> None:
    plan = _plan()
...
def test_build_candidate_cache_accepts_storage_neutral_normalized_dataset_refs() -> None:
    plan = _plan()
...
def test_build_candidate_cache_canonicalizes_mongo_attempt_refs() -> None:
    plan = _plan()
    inputs = _normalized_inputs(count=1)
    filtered = _filtered_universe(inputs)
    scoring = _scoring_result(count=1)
...
def test_build_candidate_cache_rejects_blocker_data_gap() -> None:
    plan = _plan()
    inputs = _normalized_inputs()
    filtered = _filtered_universe(inputs)
    scoring = _scoring_result()
    ...
    assert exc_info.value.code == "candidate_cache_data_gap_blocker"


def test_build_candidate_cache_accepts_disabled_strategy_warn_gap() -> None:
    plan = _plan()
    inputs = _normalized_inputs()
    filtered = _filtered_universe(inputs)
    scoring = _scoring_result()
...
def test_build_candidate_cache_accepts_count_less_than_20() -> None:
    plan = _plan()
...
def test_build_candidate_cache_accepts_count_equal_to_one() -> None:
    plan = _plan()
    inputs = _normalized_inputs(count=1)
    filtered = _filtered_universe(inputs)
    scoring = _scoring_result(count=1)
...
def test_build_candidate_cache_rejects_when_candidate_count_exceeds_20() -> None:
    plan = _plan()
    inputs = _normalized_inputs(count=21)
    filtered = _filtered_universe(inputs)
    scoring = _scoring_result(count=21)

    with pytest.raises(CandidateCacheError, match="candidate_cache_top_limit_exceeded"):
        build_candidate_cache(
            plan=plan,
            inputs=inputs,
            filtered=filtered,
...
def test_build_candidate_cache_rejects_when_candidate_count_is_zero() -> None:
    plan = _plan()
    inputs = _normalized_inputs(count=0)
    filtered = _filtered_universe(inputs)
    scoring = _scoring_result(count=0)

    with pytest.raises(CandidateCacheError, match="candidate_cache_top20_count_invalid"):
        build_candidate_cache(
            plan=plan,
            inputs=inputs,
            filtered=filtered,
...
def test_validate_candidate_cache_contract_rejects_subjective_language() -> None:
    plan = _plan()
    inputs = _normalized_inputs()
    filtered = _filtered_universe(inputs)
    scoring = _scoring_result()
    ...
    with pytest.raises(CandidateCacheError, match="candidate_cache_subjective_language_forbidden"):
        validate_candidate_cache_contract(plan=plan, draft=bad)


def test_validate_candidate_cache_contract_rejects_protocol_terms_in_json_body() -> None:
    plan = _plan()
    inputs = _normalized_inputs()
    filtered = _filtered_universe(inputs)
    scoring = _scoring_result()
    ...
    with pytest.raises(CandidateCacheError, match="candidate_cache_model_boundary_forbidden"):
        validate_candidate_cache_contract(plan=plan, draft=bad)


def test_build_candidate_cache_rejects_lineage_missing_attempts() -> None:
    plan = _plan()
    inputs = _normalized_inputs()
    filtered = _filtered_universe(inputs)
    scoring = _scoring_result()

    with pytest.raises(CandidateCacheError, match="candidate_cache_lineage_incomplete"):
        build_candidate_cache(
            plan=plan,
            inputs=inputs,
            filtered=filtered,
