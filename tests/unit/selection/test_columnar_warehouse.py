...
def test_columnar_warehouse_writes_reads_and_validates_hash(tmp_path: Path) -> None:
        ...
        (
            {
                ...
                "volume": 1000.0,
                "amount": 1000000.0,
                "dataset_ref": "mongo://normalized_datasets/daily/000001.SZ/2026-06-04",
                "source_ref": "normalized://mongo/normalized_datasets/daily/000001.SZ/2026-06-04",
            },
        )
    )
    ...
    manifest = writer.commit(
        ...
        normalized_refs=("normalized://mongo/normalized_datasets/daily/000001.SZ/2026-06-04",),
    )
    ...
    rows = warehouse.read_feature_rows(manifest=manifest)
...
def test_columnar_warehouse_rejects_tampered_manifest_file(tmp_path: Path) -> None:
    ...
    manifest = writer.commit(provider_attempt_refs=("attempt://provider/1",), normalized_refs=("normalized://mongo/ref",))
...
def test_columnar_warehouse_rejects_incomplete_coverage_manifest(tmp_path: Path) -> None:
    ...
    writer.commit(
        provider_attempt_refs=("attempt://provider/1",),
        normalized_refs=("normalized://mongo/ref",),
...
def test_columnar_warehouse_load_valid_manifest_rejects_plan_mismatch(tmp_path: Path) -> None:
    ...
    writer.commit(provider_attempt_refs=("attempt://provider/1",), normalized_refs=("normalized://mongo/ref",))
...
def test_selection_batch_uses_valid_columnar_manifest_before_data_gateway(tmp_path: Path, monkeypatch) -> None:
    ...
    assert result.attempt_refs == ("attempt://provider/columnar-cache",)
