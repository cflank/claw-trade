...
from claw_trade.selection.data_need_refresh import (
    _smoke_cli_run,
...
def test_selection_batch_facade_forwards_progress_callback(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("claw_trade.selection.data_need_refresh._fetch_selection_batch_from_data_gateway", fake_fetch)
...
def test_selection_refresh_chunks_split_across_weekends() -> None:
    needs = tuple(
        DataNeed(
            ...
            deadline_at=datetime(2026, 6, 4, tzinfo=UTC),
            consumer="select",
        )
        for day in (date(2025, 4, 18), date(2025, 4, 21), date(2025, 4, 22))
    )

    chunks = _refresh_need_chunks(needs, 5)

    assert tuple(tuple(need.time_range_start for need in chunk) for chunk in chunks) == (
        (date(2025, 4, 18),),
...
def test_selection_data_need_audit_does_not_carry_provider_candidates() -> None:
    plan = build_selection_data_need_audit(
        market=SelectionMarket.CN_A,
        profile=SelectionProfile.CN_A,
    ...
    assert plan.coverage_groups == ("cn_a_selection_batch", "universe", "daily", "fundamental")
...
def test_selection_gateway_expands_stale_all_share_batch_to_trade_date_universe_refresh(
    ...
    fake_api = _SequencedFakeDataAPI(
        (
            ...
            (refresh_result.model_copy(update={"request_id": main_request_id}),),
        )
    )
    fake_gateway = _FakeGateway(fake_api, data_need_results=((refresh_result,),))
    monkeypatch.setattr(
        "claw_trade.data_gateway._selection_batch._build_selection_gateway_context",
        lambda: fake_gateway,
    )
    ...
    assert len(fake_gateway.data_need_calls) == 1
    assert [need.instrument for need in fake_gateway.data_need_calls[0]] == ["all_a_shares"]
...
def test_selection_gateway_expands_metadata_only_coverage_gap_to_universe_refresh(
    ...
    fake_gateway = _FakeGateway(fake_api)
    monkeypatch.setattr(
        "claw_trade.data_gateway._selection_batch._build_selection_gateway_context",
        lambda: fake_gateway,
    )
    ...
    assert all(need.consumer == "select" for need in refresh_needs)
...
def test_selection_gateway_refreshes_full_request_when_metadata_integrity_fails_without_usable_rows(
    ...
    refresh_needs = tuple(need for call in fake_gateway.data_need_calls for need in call)
    assert len(refresh_needs) == 1
    assert refresh_needs[0].time_range_start == date(2026, 6, 4)
    assert refresh_needs[0].time_range_end == date(2026, 6, 4)
    assert all(need.instrument == "all_a_shares" for need in refresh_needs)
    assert all(need.consumer == "select" for need in refresh_needs)
    assert all(need.time_range_start == need.time_range_end for need in refresh_needs)
...
def test_selection_gateway_refreshes_only_metadata_integrity_mismatch_dates(
    ...
    refresh_needs = tuple(need for call in fake_gateway.data_need_calls for need in call)
    assert [need.time_range_start for need in refresh_needs] == [date(2025, 4, 17)]
    assert [need.time_range_end for need in refresh_needs] == [date(2025, 4, 17)]
    assert all(need.instrument == "all_a_shares" for need in refresh_needs)
    assert all(need.consumer == "select" for need in refresh_needs)
    assert fake_api.calls[-1][0].request_id == main_request_id
    assert fake_api.calls[-1][0].freshness_policy == "warehouse_only"
...
def _smoke_result(*, rows: int, gaps: tuple[DataGapRef, ...]) -> SelectionDataNeedResult:
    plan = build_selection_data_need_audit(
        market=SelectionMarket.CN_A,
        profile=SelectionProfile.CN_A,
        trade_date="2026-05-26",
...
def test_smoke_cli_returns_zero_for_rows_with_warn_gap_only(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr("claw_trade.selection.data_need_refresh.fetch_selection_batch_from_data_gateway", lambda _plan: result)
...
class _FakeDataAPI:
    ...
    def read_warehouse_batch(self, requests):
...
class _FakeGateway:
    def __init__(
        ...
        data_need_results: tuple[tuple[DataResult, ...], ...] = (),
    ) -> None:
        self.data_service = data_api
        self.repository = repository
        self.data_need_calls: list[tuple[DataNeed, ...]] = []
        self._data_need_results = list(data_need_results)
    ...
    def data_need_executor(self, _plan: SelectionRunPlan, needs: tuple[DataNeed, ...]) -> tuple[DataResult, ...]:
        ...
        if not self._data_need_results:
            return ()
        return self._data_need_results.pop(0)
