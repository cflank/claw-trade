...
def test_rate_limiter_window_can_start_from_run_started_at() -> None:
    run_started_at = datetime(2026, 5, 31, 12, 0, 30, tzinfo=UTC)
    clock = _Clock(run_started_at)
    limiter = RateLimiter(now_fn=clock)
    policy = RateLimitPolicy(
        window_seconds=60,
        max_requests=1,
        overflow="fail_fast",
        window_anchor=run_started_at,
    )
