...
def test_export_saved_markdown_to_pdf_does_not_write_file_when_report_has_asset_dir(tmp_path, monkeypatch) -> None:  # type: ignore[no-untyped-def]
    repo = ReportRepository()
...
def test_export_saved_markdown_to_pdf_ignores_old_pdf_directory(tmp_path, monkeypatch) -> None:  # type: ignore[no-untyped-def]
    repo = ReportRepository()
    asset_dir = tmp_path / "reports" / "assets"
    repo.save_succeeded_report(
        report_id="r-old-pdf",
    ...
    service = PdfExportService(repo, renderer=_PassRenderer(), runtime_capabilities_provider=_ready_capabilities)
    monkeypatch.setattr(
        "claw_trade.ui_backend.pdf_export_service.validate_pdf_bytes",
        lambda *_args, **_kwargs: PdfValidationResult(
            valid=True,
    ...
    record = service.export_saved_markdown_to_pdf("r-old-pdf", request_id="pdf-old")

    assert record.state == "ready"
    assert pdf_path.exists()
    assert sorted(item.name for item in pdf_dir.iterdir()) == ["pdf_restored.pdf"]
...
def test_export_report_pdf_api_is_disabled_without_rendering(tmp_path) -> None:
    repo = ReportRepository()
    repo.save_succeeded_report(
    ...
    record = service.export_report_pdf("r-pdf-cleanup", request_id="pdf-cleanup")

    assert record["state"] == "failed"
    assert record["available"] is False
    assert record["userMessage"] == "系统不保存 PDF，请使用微信转发完整报告。"
    assert not (tmp_path / "reports" / "pdf").exists()
