...
def test_confirm_scheduled_report_from_wechat_uses_logical_recipient_without_account_route() -> None:
    controller, _queue, _runner, recognizer = _build_controller()
    draft = recognizer.classify_user_intent(
        text="每天 08:00 给我 BTC 报告",
        source_message_id="m-wechat-schedule",
        settings=ReportWorkflowSettings(),
    )
    assert draft is not None
    controller.register_draft(draft)
    ...
    result = controller.confirm_intent_draft(
        request_id="c-wechat-schedule",
        draft_id=draft.draft_id,
        decision="confirm",
        origin_context_id="wechat_clawbot:old-account:sender-1",
    )
