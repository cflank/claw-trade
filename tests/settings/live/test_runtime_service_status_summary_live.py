...
def test_live_runtime_service_status_summary_uses_real_health_and_no_internal_leak(tmp_path: Path) -> None:
    project_root = Path(__file__).resolve().parents[3]
    artifact_dir = project_root / ".runtime" / "test-artifacts" / "settings-s09"
    ...
    with TestClient(app) as client:
        response = client.get("/api/ui/get-advanced-diagnostics-runtime-service-status")
    assert response.status_code == 200
    payload = response.json()
    assert payload["state"] in {"healthy", "degraded"}
    assert payload["source"] == "runtime.health.http"
    assert payload["checkedAt"]

    lowered = str(payload["userMessage"]).lower()
    assert "18789" not in lowered
    assert "1933" not in lowered
    assert "gateway" not in lowered
    assert "scope" not in lowered
    assert "openviking" not in lowered
    assert "/runtime/dev-services" not in lowered

    assert summary["state"] == payload["state"]
    (artifact_dir / "runtime-service-status-summary-live.json").write_text(
