...
from claw_trade.ui_backend.worker_chat_openclaw import OpenClawAgentChatResponse, OpenClawWorkerChatClient
...
_VISIBLE_REPLY_RETRY_SUFFIX = ":visible-reply-retry"
_VISIBLE_REPLY_REQUIREMENT = "必须输出一段用户能看到的中文回答；不要输出 NO_REPLY、ANNOUNCE_SKIP 或 REPLY_SKIP。"
...
class WorkerChatController:
    ...
    def send_worker_chat(
        ...
        *,
        ...
        reply = self._send_worker_chat_with_visible_reply_retry(
            mode=normalized_mode,
            worker_id=worker.worker_id,
            user_message=prompt,
            conversation_id=normalized_conversation_id,
    ...
    def _send_worker_chat_with_visible_reply_retry(
        self,
        *,
        mode: WorkerChatMode,
        worker_id: str,
        ...
        try:
            return self._openclaw.send_worker_chat(
                mode=mode,
                worker_id=worker_id,
            ...
            if not _is_missing_visible_reply_error(exc):
                raise
        retry_message = (
            f"{user_message}\n\n"
            "【Retry】\n"
            "上一次没有返回可显示回答。请直接回答用户问题，不要输出 NO_REPLY、ANNOUNCE_SKIP 或 REPLY_SKIP。"
        )
        return self._openclaw.send_worker_chat(
            mode=mode,
            worker_id=worker_id,
            ...
            conversation_id=conversation_id,
            idempotency_key=f"{idempotency_key}{_VISIBLE_REPLY_RETRY_SUFFIX}",
            report_id=report_id,
        )
...
def _build_worker_chat_message(
    *,
    ...
    if mode == "generic_worker_chat":
        return (
            f"你现在以「{worker_display_name}」的视角进行普通聊天。"
            "这不是报告执行，也不是报告追问；不要输出 Run ID、Profile、Status 或报告执行摘要。\n\n"
            f"{user_text}"
        )
    return (
...
def _is_missing_visible_reply_error(exc: Exception) -> bool:
    message = str(exc).strip().lower()
    return "assistant_unavailable" in message or "no visible" in message or "no_reply" in message
