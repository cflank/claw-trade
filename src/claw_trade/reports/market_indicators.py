...
def _warning_list(indicators: dict[str, object], *, ticker: str) -> list[dict[str, object]]:
    warnings: list[dict[str, object]] = []
...
def analyze_market_frame(frame: pd.DataFrame, *, ticker: str) -> AnalysisBundle:
    ...
    indicators = {
        "ma": {"ma5": _to_float(latest["ma5"]), "ma10": _to_float(latest["ma10"]), "ma20": _to_float(latest["ma20"])},
        "macd": {"macd": _to_float(latest["macd"]), "signal": _to_float(latest["signal"]), "hist": _to_float(latest["hist"])},
        "rsi": {"rsi14": _to_float(latest["rsi14"])},
        "boll": {"upper": _to_float(latest["boll_upper"]), "mid": _to_float(latest["boll_mid"]), "lower": _to_float(latest["boll_lower"])},
        "atr": {"atr14": _to_float(latest["atr14"])},
        "kdj": {"k": _to_float(latest["k"]), "d": _to_float(latest["d"]), "j": _to_float(latest["j"])},
    }
    warnings = _warning_list(indicators, ticker=ticker)
