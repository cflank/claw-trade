...
from claw_trade.data_gateway.models import GapSeverity, Readiness, ReadinessStatus
...
def _result_status(sections: tuple[AnalysisSection, ...]) -> str:
    blocking = {AnalysisState.GAP}
    if any(section.status in blocking for section in sections):
        return "insufficient"
    if any(section.status == AnalysisState.PARTIAL for section in sections):
        return "partial"
    return "ready"


def _readiness(data: CryptoLensInput) -> Readiness:
    blocking_gap_ids = tuple(gap.gap_id for gap in data.data_gaps if gap.severity == GapSeverity.FAIL)
    non_blocking_gap_ids = tuple(gap.gap_id for gap in data.data_gaps if gap.severity != GapSeverity.FAIL)
    missing_domains = tuple(
        domain
        for domain, status in data.domain_status.items()
        if status.value in {"missing", "error", "license_blocked"}
    )
    if blocking_gap_ids or missing_domains:
        status = ReadinessStatus.INSUFFICIENT
    elif non_blocking_gap_ids or any(status.value in {"partial", "insufficient", "stale"} for status in data.domain_status.values()):
        status = ReadinessStatus.PARTIAL
    else:
        status = ReadinessStatus.READY
    root_cause = "; ".join(gap.root_cause for gap in data.data_gaps[:3]) or None
    return Readiness(
        status=status,
