...
class SelectionColumnarWarehouse:
    ...
    def read_feature_rows(self, *, manifest: SelectionColumnarManifest) -> tuple[Mapping[str, object], ...]:
        if not self.validate_manifest(manifest):
        ...
        with duckdb.connect(database=":memory:") as con:
            frame = con.execute("SELECT * FROM read_parquet(?)", [paths]).fetch_df()
        return tuple(_clean_mapping(row) for row in frame.to_dict("records"))
...
def _manifest_from_payload(payload: Mapping[str, Any]) -> SelectionColumnarManifest:
    return SelectionColumnarManifest(
        ...
        provider_attempt_refs=tuple(str(item) for item in _payload_list(payload.get("provider_attempt_refs"))),
        normalized_refs=tuple(str(item) for item in _payload_list(payload.get("normalized_refs"))),
