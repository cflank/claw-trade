...
from claw_trade.guards.pm_owner import validate_pm_owner
...
def approve_worker_material(
    ...
    # PM owner 边界：只有 PM worker 读取 pm-decision.json，且字段必须和 receipt/claims 完全对齐。
    pm_guard = guard_passed(category="pm_owner")
    if call.worker_id == "portfolio_manager":
        _, pm_guard = validate_pm_owner(call=call, evidence=evidence, receipt=candidate.receipt, claims=claims)
